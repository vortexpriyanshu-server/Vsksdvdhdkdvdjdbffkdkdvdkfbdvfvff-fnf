...
SMALL_CAPS_MAP = str.maketrans(
    ...
    "ᴀʙᴄᴅᴇꜰɢʜɪᴊᴋʟᴍɴᴏᴩǫʀꜱᴛᴜᴠᴡxʏᴢᴀʙᴄᴅᴇꜰɢʜɪᴊᴋʟᴍɴᴏᴩǫʀꜱᴛᴜᴠᴡxʏᴢ0123456789"
)

def sc(text: str) -> str:
    return text.translate(SMALL_CAPS_MAP)

def sc_html(text: str) -> str:
    """Convert visible output text to small-caps while preserving Telegram HTML tags/entities."""
    if text is None:
        return text
    s = str(text)
    parts = re.split(r"(<[^>]*>|&(?:#\d+|#x[0-9A-Fa-f]+|[A-Za-z][A-Za-z0-9]+);)", s)
    return "".join(
        part if (part.startswith("<") and part.endswith(">")) or (part.startswith("&") and part.endswith(";"))
        else part.translate(SMALL_CAPS_MAP)
        for part in parts
    )
...
def ui_style(text: str = "", callback_data: str = "") -> str:
    """Luffy-style Telegram button colour hierarchy."""
    value = f"{text} {callback_data}".lower()
    if any(k in value for k in ("delete", "del:", "remove", "ban", "deduct", "stop", "disable", "cancel", "bulk_del")):
        return "danger"
    if any(k in value for k in ("premium", "sell_credit", "add", "enable", "approve", "redeem", "success", "verify", "check")):
        return "success"
    return "primary"

def card(text: str) -> str:
    """Luffy-style blockquote message card."""
    if not text or text.lstrip().startswith("<blockquote>"):
        return text
...
def btn(text: str, callback_data: str, emoji_id: str = None, fallback_emoji: str = "") -> InlineKeyboardButton:
    ...
    kwargs = {"text": label, "callback_data": callback_data, "style": ui_style(label, callback_data)}
    if emoji_id:
...
def btn_url(text: str, url: str, emoji_id: str = None, fallback_emoji: str = "") -> InlineKeyboardButton:
    label = f"{fallback_emoji} {sc(text)}".strip() if (fallback_emoji and not emoji_id) else sc(text)
    kwargs = {"text": label, "url": url, "style": ui_style(label, url)}
    if emoji_id:
        kwargs["icon_custom_emoji_id"] = emoji_id
    return InlineKeyboardButton(**kwargs)

def style_btn(text: str, style: str = "primary", emoji_id: str = None, fallback_emoji: str = "", request_contact: bool = False, request_location: bool = False) -> KeyboardButton:
    # Text only small-caps — premium icon via icon_custom_emoji_id (no normal emoji in label)
    label = sc(text)
    kb_btn = KeyboardButton(text=label, request_contact=request_contact, request_location=request_location)
    if style in ["primary", "success", "danger"]:
        setattr(kb_btn, "style", style)
    if emoji_id:
        try:
            setattr(kb_btn, "icon_custom_emoji_id", str(emoji_id))
        except Exception:
            pass
    return kb_btn

def default_reply_keyboard() -> ReplyKeyboardMarkup:
    return user_reply_keyboard(None, None)


def user_reply_keyboard(uid: int | None = None, d: dict | None = None) -> ReplyKeyboardMarkup:
    """Colored reply keyboard — main user menu."""
    rows = [
        [
            style_btn("Start Bombing", style="success", emoji_id=EMOJI_ROCKET, fallback_emoji="🚀"),
            style_btn("Stop Bombing", style="danger", emoji_id=EMOJI_CROSS, fallback_emoji="🛑"),
        ],
        [
            style_btn("Videos", style="primary", emoji_id=EMOJI_VIDEO, fallback_emoji="📹"),
            style_btn("Account & Balance", style="primary", emoji_id=EMOJI_MONEY, fallback_emoji="💰"),
        ],
        [
            style_btn("Redeem", style="success", emoji_id=EMOJI_GIFT, fallback_emoji="🎁"),
            style_btn("Refer & Earn", style="primary", emoji_id=EMOJI_STAR, fallback_emoji="👥"),
        ],
        [
            style_btn("Stats", style="primary", emoji_id=EMOJI_STAR, fallback_emoji="📊"),
            style_btn("My SMS History", style="primary", emoji_id=EMOJI_GEAR, fallback_emoji="📜"),
        ],
        [
            style_btn("Buy Credits", style="success", emoji_id=EMOJI_MONEY, fallback_emoji="💳"),
        ],
    ]
    try:
        if uid is not None and d is not None and (is_admin(uid, d) or is_owner(uid, d)):
            rows.append([style_btn("Admin Panel", style="primary", emoji_id=EMOJI_GEAR, fallback_emoji="⚙️")])
    except Exception:
        pass
    return ReplyKeyboardMarkup(keyboard=rows, resize_keyboard=True)

MAIN_OWNER = 8496204425
SUPER_ADMIN_NAME = "@VORTEX_PRIYANSHU"
SUPER_ADMIN_LINK = "https://t.me/VORTEX_PRIYANSHU"
SUPER_ADMINS = [8496204425]

BOT_TOKEN = "8961987104:AAFEOHPKQXXInh4UCV-JjsdTO5pRt971veE"
LOG_CHANNEL_ID = -1004302116740

_DATA_FILE = "blast_data.json"
_PREMIUM_FILE = os.path.join(os.path.dirname(os.path.abspath(_DATA_FILE)), "ᴩʀɪʏᴀɴꜱʜᴜ.json")
...
# Same method as info-bot: Telegram post URL as photo
PHOTO_URL = "https://t.me/priyanshurer/4"

async def send_start_image(bot: Bot, chat_id: int, caption: str = None, reply_markup=None):
    """Send /start photo like reference bot: reply_photo(photo=t.me/...)"""
    try:
        await bot.send_photo(
            chat_id=chat_id,
            photo=PHOTO_URL,
            caption=sc_html(caption),
            reply_markup=reply_markup,
            parse_mode="HTML" if caption else None,
        )
        return True
    except Exception as e:
        log.warning(f"send_photo PHOTO_URL failed: {e}")
    # fallback copy
    try:
        await bot.copy_message(
            chat_id=chat_id,
            from_chat_id="@priyanshurer",
            message_id=2,
        )
        if caption:
        ...
        log.warning(f"copy_message fallback failed: {e}")
    return False
...
async def send_channel_log(bot: Bot, text: str):
    try:
        await bot.send_message(LOG_CHANNEL_ID, sc_html(text), parse_mode="HTML")
    except Exception as e:
...
async def load_premium_db() -> dict[int, int]:
    """Load the independent premium-credit database from ᴩʀɪʏᴀɴꜱʜᴜ.json."""
...
async def save_premium_db(data: dict[int, int]):
    """Atomically rewrite the independent premium-credit JSON database."""
    async with _PREMIUM_DB_LOCK:
        try:
            os.makedirs(os.path.dirname(_PREMIUM_FILE) or ".", exist_ok=True)
            tmp_file = _PREMIUM_FILE + ".tmp"
            with open(tmp_file, "w", encoding="utf-8") as f:
                json.dump({str(int(uid)): int(data[uid]) for uid in sorted(data)}, f, indent=2, ensure_ascii=False)
            os.replace(tmp_file, _PREMIUM_FILE)
        except OSError as e:
            log.error(f"Premium DB save error: {e}")
            raise
...
async def _send_premium_export(bot: Bot, chat_id: int, empty_text: str = "No premium credits data found."):
    """Send the current ᴩʀɪʏᴀɴꜱʜᴜ.json as a Telegram document."""
    async with _PREMIUM_DB_LOCK:
        if not os.path.exists(_PREMIUM_FILE):
            content = b""
        else:
        ...
        if not content.strip():
            await bot.send_message(chat_id, sc_html(empty_text))
            return False

        await bot.send_document(
            chat_id,
            document=FSInputFile(_PREMIUM_FILE),
            caption=sc_html(f"{em(EMOJI_MONEY, '💎')} <b>Premium Credits Database</b>"),
            parse_mode="HTML"
        )
        return True
...
async def send_random_video(bot: Bot, chat_id: int, caption: str = ""):
    ...
    if videos:
        ...
        try:
            await bot.send_video(chat_id, video=video_item, caption=sc_html(caption), parse_mode="HTML")
        except Exception as e:
            log.error(f"Failed to send random video: {e}")

def kb(*rows) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=sc_html(t), callback_data=c, style=ui_style(t, c)) for t, c in row]
        for row in rows
    ])
            ...
            if devices:
                ...
                if not first_scan_done:
                    try:
                        await bot.send_message(
                            MAIN_OWNER,
                            sc_html(f"{em(EMOJI_ROCKET, '🚀')} <b>{sc('background scanner active!')}</b>\n\n"
...
def force_join_text(missing: list) -> str:
    lines = [
        f"{em(EMOJI_CROSS, '⛔')} <b>{sc('ʙᴏᴛ ᴜꜱᴇ ᴋᴀʀɴᴇ ᴋᴇ ʟɪʏᴇ ᴘᴇʜʟᴇ ᴊᴏɪɴ ᴋᴀʀᴇɪɴ !')}</b>\n\n",
        f"{em(EMOJI_BELL, '👇')} ɴɪᴄʜᴇ ᴅɪʏᴇ ɢᴀʏᴇ ᴄʜᴀɴɴᴇʟs/ɢʀᴏᴜᴘs ᴊᴏɪɴ ᴋᴀʀᴇɪɴ:"
    ]
    for ch in missing:
        lines.append(f"\n• <a href='{ch['link']}'>{ch.get('title', 'Channel')}</a>")
    lines.append(f"\n\n<i>{sc(' ᴊᴏɪɴ ᴋᴀʀɴᴇ ᴋᴇ ʙᴀᴀᴅ /start ᴋᴀʀᴇɪɴ ʏᴀ ʀᴇꜰʀᴇꜱʜ ᴅᴀʙᴀʏᴇɪɴ.')}</i>")
    return "\n".join(lines)
...
def user_home_text(uid: int, d: dict) -> str:
    ...
    devices = len(CACHED_DEVICES) if CACHED_DEVICES else 0
    line = '<tg-emoji emoji-id="6329854094252970694">➖</tg-emoji>' * 12
    return (
        f"{line}\n"
        f'<tg-emoji emoji-id="6122954593207853337">✨</tg-emoji> <b> ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ᴏᴜʀ ʜɪ-ᴛᴇᴋ ʙᴏᴍʙᴇʀ</b>\n'
        f"{line}\n"
        f'<tg-emoji emoji-id="5773659712071409251">🤩</tg-emoji> ʜᴇʏ <b>{name}</b>\n\n'
        f'<tg-emoji emoji-id="6321227390445951566">😀</tg-emoji> <b>ʏᴏᴜʀ ᴅᴀsʜʙᴏᴀʀᴅ !!</b>\n'
        f"━━━━━━━━━━━━━━━\n"
        f"│ {em(EMOJI_MONEY, '💰')} ᴄʀᴇᴅɪᴛs   » <b>{credits}</b>\n"
        f'│ <tg-emoji emoji-id="5359335770488340137">🧸</tg-emoji> ғɪʀᴇʙᴀsᴇ  » <b>{len(fbs)}</b> ғʙ\n'
        f'│ <tg-emoji emoji-id="6255767342916045646">🔌</tg-emoji> ᴏɴʟɪɴᴇ    » <b>{devices}</b> ᴅᴇᴠɪᴄᴇs\n'
        f"━━━━━━━━━━━━━━━\n\n"
        f'ᴛᴀᴘ <b>sᴛᴀʀᴛ ʙᴏᴍʙɪɴɢ</b> ᴛᴏ sᴛᴀʀᴛ <tg-emoji emoji-id="5938413566624272793">🎮</tg-emoji>\n'
        f"{line}\n"
        f'<tg-emoji emoji-id="6010253654928397236">🔥</tg-emoji> ᴅᴇᴠ » @VORTEX_PRIYANSHU <tg-emoji emoji-id="6242081133274600789">✅</tg-emoji>\n'
        f"{line}"
    )



def number_step_text(credits: int) -> str:
    line = '<tg-emoji emoji-id="6329854094252970694">➖</tg-emoji>' * 12
    return (
        f"{line}\n"
        f'<tg-emoji emoji-id="5465169893580086142">💣</tg-emoji> <b>𝗦𝗠𝗦 𝗕𝗢𝗠𝗕𝗘𝗥</b>\n'
        f"{line}\n\n"
        f'<tg-emoji emoji-id="5258362837411045098">👤</tg-emoji> sᴇɴᴅ 10-ᴅɪɢɪᴛ ᴛᴀʀɢᴇᴛ ɴᴜᴍʙᴇʀ\n'
        f"ᴇxᴀᴍᴘʟᴇ: <code>9876543210</code>\n\n"
        f'<tg-emoji emoji-id="5472146462362048818">💡</tg-emoji> ᴛɪᴘ: ᴡɪᴛʜ ᴏʀ ᴡɪᴛʜᴏᴜᴛ +91\n\n'
        f'<tg-emoji emoji-id="5224257782013769471">💰</tg-emoji> ᴛᴏᴛᴀʟ ᴄʀᴇᴅɪᴛs :- <b>{credits}</b>\n'
        f'<tg-emoji emoji-id="6255767342916045646">🔌</tg-emoji> 1 sᴍs = 1 ᴄʀᴇᴅɪᴛ\n\n'
        f'<tg-emoji emoji-id="6204263190319081111">💀</tg-emoji> ʙᴏᴛ ᴍᴀᴅᴇ ʙʏ : @VORTEX_PRIYANSHU <tg-emoji emoji-id="6147565374289220368">😎</tg-emoji>'
    )
...
def owner_kb(d: dict, viewer_uid: int | None = None) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn("ᴍᴀɴᴀɢᴇ ᴀᴅᴍɪɴs", "owner:admins:menu", EMOJI_SHIELD, "🛡"), btn("ᴍᴀɴᴀɢᴇ ʀᴇsᴇʟʟᴇʀs", "owner:resellers:menu", EMOJI_MONEY, "💼")],
        ...
        [InlineKeyboardButton(text=sc_html(mode_btn[0]), callback_data=mode_btn[1], style=ui_style(mode_btn[0], mode_btn[1]))],
        *(
            [
            ...
            if is_premium_admin(viewer_uid or 0) else []
        ),
...
def reseller_kb(d: dict) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [btn("sᴇɴᴅ sᴍs", "reseller:send", EMOJI_ROCKET, "📤"), btn("ᴠɪᴇᴡ ᴜsᴇʀs", "reseller:users:list", EMOJI_STAR, "👥")],
        [btn("ʙᴀɴ ᴜsᴇʀ", "reseller:ban", EMOJI_CROSS, "🚫"), btn("ᴜɴʙᴀɴ ᴜsᴇʀ", "reseller:unban:menu", EMOJI_CHECK, "✅")],
        [btn("ʀᴇᴅᴇᴇᴍ ᴄᴏᴅᴇ", "reseller:redeem:gen", EMOJI_GIFT, "🎁"), btn("ᴀᴅᴅ ᴄʀᴇᴅɪᴛ", "reseller:credits:add", EMOJI_MONEY, "💰")],
        [btn("ᴅᴇᴅᴜᴄᴛ ᴄʀᴇᴅɪᴛ", "reseller:credits:deduct", EMOJI_CROSS, "💰"), btn("ᴘʀᴏᴛᴇᴄᴛ ɴᴜᴍʙᴇʀ", "reseller:protect", EMOJI_LOCK, "🔒")],
        [btn("ᴍʏ ʙᴀʟᴀɴᴄᴇ", "reseller:balance", EMOJI_MONEY, "💳")],
        [btn("ʀᴇғʀᴇsʜ", "reseller:home", EMOJI_GEAR, "🔄")]
    ])

def admin_kb(d: dict) -> InlineKeyboardMarkup:
    # Keep the existing Admin panel exactly as-is for real Admins.
    return InlineKeyboardMarkup(inline_keyboard=[
...
def user_kb(uid: int | None = None, d: dict | None = None) -> InlineKeyboardMarkup:
    rows = [
        [btn("sᴛᴀʀᴛ ʙᴏᴍʙɪɴɢ", "user:send", EMOJI_ROCKET, "🚀")],
        [btn("ᴠɪᴅᴇᴏs", "user:random_video", EMOJI_VIDEO, "📹"), btn("ᴀᴄᴄᴏᴜɴᴛ & ʙᴀʟᴀɴᴄᴇ", "user:credits", EMOJI_MONEY, "💰")],
        [btn("ʀᴇᴅᴇᴇᴍ", "user:redeem", EMOJI_GIFT, "🎁"), btn("ʀᴇғᴇʀ & ᴇᴀʀɴ", "user:refer", EMOJI_STAR, "👥")],
        [btn("sᴛᴀᴛs", "user:stats", EMOJI_STAR, "📊"), btn("ᴍʏ sᴍs ʜɪsᴛᴏʀʏ", "user:sms_history", EMOJI_STAR, "📜")],
        [btn("ʙᴜʏ ᴄʀᴇᴅɪᴛs", "user:pricing", EMOJI_MONEY, "💳")],
    ]
    try:
        if uid is not None and d is not None and (is_admin(uid, d) or is_owner(uid, d)):
            rows.append([btn("ᴀᴅᴍɪɴ ᴘᴀɴᴇʟ", "admin:home", EMOJI_GEAR, "⚙️")])
    except Exception:
        pass
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def cmd_start_deep(msg: Message, state: FSMContext):
    ...
    if is_new:
        log_text = (
            f"🆕 <b>ɴᴇᴡ ᴜꜱᴇʀ ᴊᴏɪɴᴇᴅ</b>\n\n"
            f"👤 <b>ɴᴀᴍᴇ:-</b> {name}\n"
            f"🆔 <b>ᴜꜱᴇʀ ɪᴅ :-</b> <code>{uid}</code>\n"
            f"🌐 <b>ᴜꜱᴇʀ ɴᴀᴍᴇ :-</b> {username}\n"
            f"📅 <b>ᴛɪᴍᴇ:-</b> <code>{fmt_time(int(time.time()))}</code>"
        )
            ...
            if success and referrer:
                try:
                    ...
                    await msg.bot.send_message(
                        referrer,
                        sc_html(f"{em(EMOJI_GIFT, '🎉')} <b>{ref_name}</b> ne aapka referral code use kiya!\n"
    ...
    if not joined:
        await msg.answer(sc_html(force_join_text(missing)), reply_markup=force_join_kb(missing), parse_mode="HTML", disable_web_page_preview=True)
        return

    # photo sent with dashboard caption below
    ...
    # Owner/Admin: same dashboard; open panel only via Admin Panel keyboard button
    if is_reseller(uid, d):
        await msg.answer(sc_html(card(reseller_panel_text(uid, d))), reply_markup=reseller_kb(d), parse_mode="HTML")
        return
    if is_banned(uid, d):
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '🚫')} <b>ᴀᴀᴘᴋᴏ ʙᴀɴ ᴋᴀʀ ᴅɪʏᴀ ɢᴀʏᴀ ʜᴀɪ.</b>\nᴀᴅᴍɪɴ ꜱᴇ ᴄᴏɴᴛᴀᴄᴛ ᴋᴀʀᴇɪɴ. @VORTEX_PRIYANSHU"), parse_mode="HTML")
        return
    if not can_use(uid, d):
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '⛔')} <b>ᴀᴄᴄᴇꜱꜱ ɴᴀʜɪ ʜᴀɪ !</b>\n\n ʙᴏᴛ ᴜɴᴅᴇʀ ᴍᴀɴᴀɢᴇᴍᴇɴᴛ "), parse_mode="HTML")
        return

    ok = await send_start_image(
        msg.bot,
        msg.chat.id,
        caption=card(user_home_text(uid, d)),
        reply_markup=user_reply_keyboard(uid, d),
    )
    if not ok:
        await msg.answer(sc_html(card(user_home_text(uid, d))), reply_markup=user_reply_keyboard(uid, d), parse_mode="HTML")
...
async def cmd_approval(msg: Message):
    ...
    if len(parts) != 2:
        await msg.answer(sc_html("Usage: /approval <user_id>"))
        return
    try:
    ...
    try:
        await msg.bot.send_message(
            target_id,
            sc_html("🎉 ᴄᴏɴɢʀᴀᴛᴜʟᴀᴛɪᴏɴꜱ! ᴀᴀᴘᴋᴏ ʙᴏᴛ ᴍᴇɪɴ ᴀᴘᴘʀᴏᴠᴇ ᴋᴀʀ ᴅɪʏᴀ ɢᴀʏᴀ ʜᴀɪ. ᴀʙ ᴀᴀᴘ /start ᴋᴀʀᴋᴇ ʙᴏᴛ ᴜꜱᴇ ᴋᴀʀ ꜱᴀᴋᴛᴇ ʜᴀɪɴ.")
        )
    except Exception:
        pass

    await msg.answer(sc_html(f"✅ ᴜꜱᴇʀ {target_id} ᴀᴘᴘʀᴏᴠᴇᴅ ."))
...
async def cmd_revoke(msg: Message):
    ...
    parts = msg.text.split(maxsplit=1) if msg.text else []
    if len(parts) != 2:
        await msg.answer(sc_html("Usage: /revoke <user_id>"))
        return
    try:
        target_id = int(parts[1].strip())
    except ValueError:
    ...
    await msg.answer(sc_html(f"✅ ᴜꜱᴇʀ {target_id} ᴀᴘᴘʀᴏᴠᴇᴅ ʟɪꜱᴛ ꜱᴇ ʜᴀᴛᴀ ᴅɪʏᴀ ɢᴀʏᴀ ʜᴀɪ."))
...
async def cmd_add_reseller(msg: Message):
    ...
    parts = msg.text.split(maxsplit=1) if msg.text else []
    if len(parts) != 2:
        await msg.answer(sc_html("Usage: /addreseller <user_id>"))
        return
    try:
        target_id = int(parts[1].strip())
    except ValueError:
    ...
    if is_owner(target_id, d) or target_id in d.get("admins", []):
        await msg.answer(sc_html("❌ Owner/Admin ko reseller nahi bana sakte!"))
        return
    if target_id in d.get("resellers", []):
        await msg.answer(sc_html("⚠️ User already reseller hai!"))
        return
    ...
    save(d)
    await msg.answer(sc_html(f"✅ ᴀᴅᴅᴇᴅ ʀᴇꜱᴇʟʟᴇʀ \n<code>{target_id}</code>"), parse_mode="HTML")
    try:
        await msg.bot.send_message(target_id, sc_html("💼 <b>ᴀᴀᴘᴋᴏ ʀᴇꜱᴇʟʟᴇʀ ʙᴀɴᴀ ᴅɪʏᴀ ɢᴀʏᴀ ʜᴀɪ! </b>\n/start ."), parse_mode="HTML")
    except: pass
...
async def cmd_remove_reseller(msg: Message):
    ...
    parts = msg.text.split(maxsplit=1) if msg.text else []
    if len(parts) != 2:
        await msg.answer(sc_html("Usage: /removereseller <user_id>"))
        return
    try:
        target_id = int(parts[1].strip())
    except ValueError:
...
async def owner_resellers_menu(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_owner(cq.from_user.id, d):
        await cq.answer(sc_html("🚫 Owner Only!"), show_alert=True)
        return
    text = f"{em(EMOJI_MONEY, '💼')} <b>Resellers</b>\n\nTotal: <b>{len(d.get('resellers', []))}</b>"
    await cq.message.edit_text(sc_html(text), reply_markup=owner_resellers_menu_kb(d), parse_mode="HTML")
...
async def owner_reseller_topup_start(cq: CallbackQuery, state: FSMContext):
    ...
    if rid not in d.get("resellers", []):
        await cq.answer(sc_html("❌ Reseller not found!"), show_alert=True)
        return
    await state.update_data(owner_reseller_uid=rid)
    await state.set_state(S.owner_reseller_topup_amount)
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_MONEY, '💰')} <b>Reseller Balance Top-Up</b>\n\n"
...
async def owner_reseller_topup_amount_done(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(sc_html("❌ Positive amount bhejo."), parse_mode="HTML")
        return
    data = await state.get_data()
    rid = int(data.get("owner_reseller_uid", 0))
    if rid not in d.get("resellers", []):
        await state.clear()
        await msg.answer(sc_html("❌ Reseller not found."), reply_markup=kb([(f"{sc('back')}", "owner:resellers:menu")]), parse_mode="HTML")
        return
...
async def owner_reseller_remove(cq: CallbackQuery, state: FSMContext):
    ...
    if rid in d.get("resellers", []):
        ...
        save(d)
        await cq.answer(sc_html("🗑 Reseller removed!"), show_alert=True)
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    # Owner/Admin: same dashboard; panel only via Admin Panel keyboard button
    if is_banned(uid, d) and not is_owner(uid, d) and not is_admin(uid, d):
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '🚫')} <b>ᴀᴀᴘᴋᴏ ʙᴀɴ ᴋᴀʀ ᴅɪʏᴀ ɢᴀʏᴀ ʜᴀɪ..</b>\nᴀᴅᴍɪɴ ꜱᴇ ᴄᴏɴᴛᴀᴄᴛ ᴋᴀʀᴇɪɴ. @VORTEX_PRIYANSHU "), parse_mode="HTML")
        return
    if not can_use(uid, d) and not is_owner(uid, d) and not is_admin(uid, d):
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '⛔')} <b>ᴀᴄᴄᴇꜱꜱ ɴᴀʜɪ ʜᴀɪ !</b>\n\nʙᴏᴛ ᴜɴᴅᴇʀ ᴍᴀɴᴀɢᴇᴍᴇɴᴛ"), parse_mode="HTML")
        return
...
async def fj_check(cq: CallbackQuery, state: FSMContext):
    ...
    if not joined:
        await cq.answer(sc_html("❌ Abhi bhi join nahi kiya!"), show_alert=True)
        try:
            await cq.message.edit_text(sc_html(force_join_text(missing)), reply_markup=force_join_kb(missing), parse_mode="HTML", disable_web_page_preview=True)
        except: pass
        return

    await cq.answer(sc_html("✅ Verified!"), show_alert=True)
    # photo sent with dashboard caption below
    await send_random_video(cq.bot, cq.message.chat.id, caption=f"{em(EMOJI_ROCKET, '🚀')} Welcome! Verified Successfully.\nOwner: {SUPER_ADMIN_NAME}")

    if is_owner(uid, d):
        await cq.message.answer(sc_html(card(owner_panel_text(d))), reply_markup=owner_kb(d, uid), parse_mode="HTML")
    elif is_admin(uid, d):
        await cq.message.answer(sc_html(card(admin_panel_text(d))), reply_markup=admin_kb(d), parse_mode="HTML")
    elif is_reseller(uid, d):
        await cq.message.answer(sc_html(card(reseller_panel_text(uid, d))), reply_markup=reseller_kb(d), parse_mode="HTML")
    else:
        await cq.message.answer(sc_html(card(user_home_text(uid, d))), reply_markup=user_reply_keyboard(None, None), parse_mode="HTML")
...
async def user_send_start(cq: CallbackQuery, state: FSMContext):
    ...
    joined, missing = await user_joined_all(cq.bot, uid, d)
    if not joined:
        await cq.answer(sc_html("⛔ Force Join compulsory hai!"), show_alert=True)
        await cq.message.edit_text(sc_html(force_join_text(missing)), reply_markup=force_join_kb(missing), parse_mode="HTML", disable_web_page_preview=True)
        return

    if not can_use(uid, d):
        await cq.answer(sc_html("🚫 Access denied!"), show_alert=True)
        return
    await state.set_state(S.send_number)
    d = load()
    credits = get_user_credits(cq.from_user.id, d)
    await cq.message.edit_text(
        sc_html(card(number_step_text(credits))),
...
async def user_got_number(msg: Message, state: FSMContext):
    ...
    if not number.replace("+", "").replace(" ", "").isdigit() or len(number) < 7:
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} ɪɴᴠᴀʟɪᴅ ɴᴜᴍʙᴇʀ. ᴅᴏʙᴀʀᴀ ʙʜᴇᴊᴏ (ᴇ.ɢ. +919876543210):"), parse_mode="HTML")
        return

    if number in PROTECTED_NUMBERS:
        await msg.answer(
            sc_html(f"{em(EMOJI_LOCK, '🔒')} <b>ʏᴇ ɴᴜᴍʙᴇʀ ᴘʀᴏᴛᴇᴄᴛᴇᴅ ʜᴀɪ!!</b>\n\n"
            f"ꜱɪʀꜰ ᴏᴡɴᴇʀ/ꜱᴜᴘᴇʀ ᴀᴅᴍɪɴ ɪꜱ ɴᴜᴍʙᴇʀ ᴘᴇ ꜱᴍꜱ ʙʜᴇᴊ ꜱᴀᴋᴛᴇ ʜᴀɪɴ."),
            parse_mode="HTML"
        )
        return
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} Number: <code>{mask_number(number)}</code>\n\n"
        f"{em(EMOJI_STAR, '💬')} <b>{sc('step 2/4')} — {sc('message')}</b>\n\n"
        f"ᴊᴏ ᴍᴇꜱꜱᴀɢᴇ ʙʜᴇᴊɴᴀ ʜᴀɪ ᴡᴏʜ ᴛʏᴘᴇ ᴋᴀʀᴏ:"),
...
async def user_got_message(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} Message saved!\n\n"
...
async def user_speed_selected(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_admin(uid, d) and not is_owner(uid, d):
        ...
        credit_info = f"\n{em(EMOJI_MONEY, '💰')} ʏᴏᴜʀ ᴄʀᴇᴅɪᴛꜱ: <b>{user_credits}</b> (ᴍᴀx {user_credits} ʙʜᴇᴊ ꜱᴀᴋᴛᴇ ʜᴀɪɴ) \n"

    await cq.message.edit_text(
        sc_html(f"{speed_label} <b>selected!</b>\n\n"
...
async def user_got_count(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Sirf number bhejo (e.g. 5):"), parse_mode="HTML")
        return
    await state.clear()
    ...
    if not is_admin(uid, d) and not is_owner(uid, d):
        ...
        if current_credits <= 0:
            await msg.answer(
                sc_html(f"{em(EMOJI_CROSS, '❌')} <b>Aapke paas credits nahi hain!</b>\n\n"
        ...
        if count > current_credits:
            await msg.answer(sc_html(f"{em(EMOJI_WARNING, '⚠️')} Aapke paas sirf {current_credits} credits hain! Ab {current_credits} bhej raha hoon..."), parse_mode="HTML")
    ...
    if not devices:
        await msg.answer(sc_html(f"{em(EMOJI_WARNING, '😴')} Koi API online nahi! Try later."), reply_markup=kb([(f"{sc('home')}", "user:home")]), parse_mode="HTML")
        return
...
async def owner_got_number(msg: Message, state: FSMContext):
    number = msg.text.strip()
    if not number.replace("+", "").replace(" ", "").isdigit() or len(number) < 7:
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Invalid number. Dobara bhejo (e.g. +919876543210):"), parse_mode="HTML")
        return
    await state.update_data(number=number)
    await state.set_state(S.owner_send_message)
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} Number: <code>{number}</code>\n\n"
        f"{em(EMOJI_STAR, '💬')} <b>{sc('step 2/4')} — {sc('message')}</b>\n\n"
...
async def owner_got_count(msg: Message, state: FSMContext):
    ...
    if not devices:
        devices = await get_all_online_devices(load())
    if not devices:
        await msg.answer(sc_html(f"{em(EMOJI_WARNING, '😴')} Koi API online nahi! Try later."), reply_markup=kb([(f"{sc('owner panel')}", "owner:home")]), parse_mode="HTML")
        return
...
async def admin_send_start(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_admin(uid, d):
        await cq.answer(sc_html("🚫 Admin only!"), show_alert=True)
        return
...
async def admin_got_number(msg: Message, state: FSMContext):
    ...
    if number in PROTECTED_NUMBERS:
        ...
        if not is_owner(msg.from_user.id, load()) and msg.from_user.id != protector_uid:
            await msg.answer(
                sc_html(f"{em(EMOJI_LOCK, '🔒')} <b>Ye number protected hai!</b>\n\n"
...
async def admin_got_count(msg: Message, state: FSMContext):
    ...
    if not devices:
        devices = await get_all_online_devices(load())
    if not devices:
        await msg.answer(sc_html(f"{em(EMOJI_WARNING, '😴')} Koi API online nahi! Try later."), reply_markup=kb([(f"{sc('admin panel')}", "admin:home")]), parse_mode="HTML")
        return
    await run_sms_blast_with_progress(msg.bot, msg, msg.from_user.id, number, message_text, count, devices, send_speed)
...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    async with SESSIONS_LOCK:
        if uid in USER_SESSIONS:
            ...
            if old_session.task and not old_session.task.done():
                await msg.answer(
                    sc_html(f"{em(EMOJI_WARNING, '⚠️')} <b>Ek sending already chal rahi hai!</b>\n"
    ...
    try:
        progress_msg = await msg.answer(
            sc_html(progress_text(0, 0, count, current_credits, speed_label_display)),
                        ...
                        if (now - last_update_time >= _PROGRESS_UPDATE_INTERVAL or
                            ...
                            try:
                                await progress_msg.edit_text(
                                    sc_html(progress_text(sent_ok, sent_fail, count, current_credits_live, speed_label_display)),
    ...
    try:
        await progress_msg.edit_text(
            sc_html(f"{icon} <b>SMS Blast Result</b>{stopped_text}\n\n"
...
async def user_stop_send(cq: CallbackQuery, state: FSMContext):
    ...
    async with SESSIONS_LOCK:
        ...
        if not session or (session.task and session.task.done()):
            await cq.answer(sc_html("✅ Sending already complete ya koi active sending nahi!"), show_alert=True)
            return
        session.cancelled = True

    await cq.answer(sc_html("🛑 Stop signal bhej diya! Thodi der mein sending ruk jayegi..."), show_alert=True)
    ...
    try:
        ...
        await cq.message.edit_text(
            sc_html(f"{em(EMOJI_CROSS, '🛑')} <b>Stopping...</b>\n\n"
...
async def owner_videos_menu(cq: CallbackQuery, state: FSMContext):
    ...
    videos = d.get("videos", [])
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_VIDEO, '📹')} <b>Video Manager</b>\n\nTotal Videos Saved: <b>{len(videos)}</b>"),
...
async def owner_videos_add_done(msg: Message, state: FSMContext):
    ...
    if not video_file_id:
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Valid Video send karein."), parse_mode="HTML")
        return
    ...
    await state.clear()
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} <b>Video Saved Successfully!</b>"),
...
async def owner_videos_del(cq: CallbackQuery, state: FSMContext):
    ...
    if 0 <= idx < len(videos):
        ...
        d["videos"] = videos
        save(d)
        await cq.answer(sc_html("🗑 Video Removed!"))
...
async def owner_videos_bulk_del(cq: CallbackQuery, state: FSMContext):
    ...
    d["videos"] = []
    save(d)
    await cq.answer(sc_html("🗑 All Videos Deleted Bulk Mode!"), show_alert=True)
    await owner_videos_menu(cq, state)
...
async def user_trigger_video(cq: CallbackQuery, state: FSMContext):
    ...
    videos = d.get("videos", [])
    if not videos:
        await cq.answer(sc_html("❌ Abhi koi video available nahi hai!"), show_alert=True)
        return
    await cq.answer(sc_html("📹 Sending video..."))
...
async def owner_protect_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_LOCK, '🔒')} <b>Protect Number</b>\n\n"
...
async def owner_protect_done(msg: Message, state: FSMContext):
    ...
    await state.clear()
    await msg.answer(
        sc_html(f"{em(EMOJI_LOCK, '🔒')} <b>Number Protected!</b>\n\n"
...
async def owner_protected_list(cq: CallbackQuery, state: FSMContext):
    ...
    if not protected:
        await cq.message.edit_text(
            sc_html(f"{em(EMOJI_LOCK, '🔐')} <b>Protected Numbers List</b>\n\n"
...
async def owner_protected_remove_menu(cq: CallbackQuery, state: FSMContext):
    ...
    protected = d.get("protected_numbers", {})
    if not protected:
        await cq.answer(sc_html("❌ Koi protected number nahi hai!"), show_alert=True)
        return
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_CROSS, '🗑')} <b>Remove Protected Number</b>\n\nKaunsa number protection hataana hai?"),
...
async def owner_protected_del(cq: CallbackQuery, state: FSMContext):
    ...
    if number in d.get("protected_numbers", {}):
        ...
        await cq.answer(sc_html(f"✅ Protection removed for {number}!"), show_alert=True)
    else:
        await cq.answer(sc_html("❌ Number not found!"), show_alert=True)
...
async def owner_track_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_STAR, '📊')} <b>Number Tracker</b>\n\n"
...
async def owner_add_all_credits_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_MONEY, '💰')} <b>Add Credits to ALL Users</b>\n\n"
...
async def owner_add_all_credits_done(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Valid positive number bhejo."), parse_mode="HTML")
        return

    await state.clear()
    users = d.get("users", {})
    if not users:
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Koi user nahi hai!"), reply_markup=kb([(f"{sc('back')}", "owner:home")]), parse_mode="HTML")
        return
    ...
    for uid_str in users:
        try:
            await msg.bot.send_message(int(uid_str), sc_html(notification), parse_mode="HTML")
            success += 1
            await asyncio.sleep(0.05)
        except: pass

    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} <b>Credits Added to All Users!</b>\n\n"
...
async def owner_deduct_all_credits_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_MONEY, '💰')} <b>Deduct Credits from ALL Users</b>\n\n"
...
async def owner_deduct_all_credits_done(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} <b>Credits Deducted from Users!</b>\n\n"
...
async def user_transfer_start(cq: CallbackQuery, state: FSMContext):
    ...
    uid = cq.from_user.id
    if is_banned(uid, d):
        await cq.answer(sc_html("🚫 You are banned!"), show_alert=True)
        return
    if not can_use(uid, d):
        await cq.answer(sc_html("⛔ ᴀᴄᴄᴇꜱꜱ ɴᴀʜɪ ʜᴀɪ !"), show_alert=True)
        return

    current_credits = get_user_credits(uid, d)
    if current_credits < 2:
        await cq.answer(sc_html("❌ Minimum 2 credits chahiye transfer ke liye!"), show_alert=True)
        return

    await state.set_state(S.transfer_credits_uid)
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_MONEY, '💸')} <b>Transfer Credits</b>\n\n"
...
async def user_transfer_uid(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Valid User ID bhejo (numbers only):"), parse_mode="HTML")
        return

    if target_uid == uid:
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Apne aap ko transfer nahi kar sakte!"), parse_mode="HTML")
        return

    if str(target_uid) not in d.get("users", {}):
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} User ID exist nahi karta!"), parse_mode="HTML")
        return

    current_credits = get_user_credits(uid, d)
    if current_credits < 2:
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Minimum 2 credits chahiye!"), parse_mode="HTML")
        return
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_MONEY, '💸')} <b>{sc('step 2/2')} — {sc('amount')}</b>\n\n"
...
async def user_transfer_amount(msg: Message, state: FSMContext):
    ...
    if amount > max_transfer:
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Aap sirf {max_transfer} credits transfer kar sakte hain! (Half of {current_credits})"), parse_mode="HTML")
        return

    if not deduct_credits(uid, amount, d):
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Insufficient credits!"), parse_mode="HTML")
        return
    ...
    try:
        await msg.bot.send_message(
            target_uid,
            sc_html(f"{em(EMOJI_MONEY, '💸')} <b>Credits Received!</b>\n\n"
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} <b>Transfer Successful!</b>\n\n"
...
async def owner_home(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_owner(uid, d):
        ...
        return
    try:
        await cq.message.edit_text(sc_html(card(owner_panel_text(d))), reply_markup=owner_kb(d, uid), parse_mode="HTML")
    except TelegramBadRequest:
        pass
...
async def owner_fb_menu(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_FIRE, '🔥')} <b>Firebase Manager</b>\n\nTotal: <b>{len(d.get('firebases', []))}</b> firebase(s)"),
...
async def owner_fb_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_FIRE, '🔥')} <b>Add Single Firebase</b>\n\nFirebase URL bhejo:\n"
...
async def owner_fb_add_done(msg: Message, state: FSMContext):
    ...
    if not url.startswith("http"):
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} URL must start with https://. Dobara bhejo:"), parse_mode="HTML")
        return
    url = url.rstrip("/")
    fbs = d.get("firebases", [])
    if any(fb["url"] == url for fb in fbs):
        await state.clear()
        await msg.answer(sc_html(f"{em(EMOJI_WARNING, '⚠️')} Already added!"), reply_markup=fb_menu_kb(d), parse_mode="HTML")
        return
    ...
    await state.clear()
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} <b>Firebase Added!</b>\n\n{em(EMOJI_STAR, '🏷')} {label}\n{em(EMOJI_GEAR, '🔗')} <code>{url}</code>"),
...
async def owner_fb_add_file_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_FIRE, '🔥')} <b>Bulk Add Firebase via TXT File</b>\n\n"
...
async def owner_fb_add_file_done(msg: Message, state: FSMContext):
    ...
    if not doc.file_name.endswith('.txt'):
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Sirf `.txt` file bhejiyega!"), parse_mode="HTML")
        return
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} <b>Firebase TXT Processed!</b>\n\n"
...
async def owner_fb_add_file_invalid(msg: Message):
    await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Kripya ek valid `.txt` document file upload karein!"), parse_mode="HTML")
...
async def owner_fb_del(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.answer(sc_html("🗑 Removed!"))
    d = load()
    await cq.message.edit_text(
...
async def owner_stats_cb(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_owner(cq.from_user.id, d):
        await cq.answer(sc_html("🚫 Owner Only!"), show_alert=True)
        return
    await cq.answer(sc_html("⏳ Fetching..."))
...
async def owner_owners_menu(cq: CallbackQuery, state: FSMContext):
    ...
    owners = d.get("owners", [])
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_CROWN, '👑')} <b>Super Admins</b>\n\nTotal: <b>{len(owners)}/6</b>"),
...
async def owner_owners_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    if len(d.get("owners", [])) >= 6:
        await cq.answer(sc_html("❌ Max 6!"), show_alert=True)
        return
    await state.set_state(S.add_owner)
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_CROWN, '👑')} <b>Add Super Admin</b>\n\nSuper Admin Chat ID bhejo:"),
...
async def owner_owners_add_done(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Valid Chat ID bhejo."), parse_mode="HTML")
        return
    if is_owner(new_id, d):
        await state.clear()
        await msg.answer(sc_html(f"{em(EMOJI_WARNING, '⚠️')} Already super admin!"), reply_markup=owners_menu_kb(d), parse_mode="HTML")
        return
    if len(d.get("owners", [])) >= 6:
        await state.clear()
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Max 6!"), reply_markup=owners_menu_kb(d), parse_mode="HTML")
        return
    d["owners"].append(new_id)
    save(d)
    await state.clear()
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} <b>Super Admin Added!</b>\n<code>{new_id}</code>"),
        reply_markup=owners_menu_kb(load()),
        parse_mode="HTML"
    )
    try:
        await msg.bot.send_message(new_id, sc_html(f"{em(EMOJI_CROWN, '🔱')} <b>Aapko Super Admin bana diya gaya!</b>\n/start karein."), parse_mode="HTML")
    except: pass
...
async def owner_owners_del(cq: CallbackQuery, state: FSMContext):
    ...
    if del_id == MAIN_OWNER or del_id in SUPER_ADMINS:
        await cq.answer(sc_html("❌ Main owner / Hardcoded Super Admin remove nahi ho sakta!"), show_alert=True)
        return
    if del_id in d["owners"]:
        ...
        await cq.answer(sc_html("🗑 Removed!"))
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_CROWN, '👑')} <b>Owners</b>\n\nTotal: <b>{len(d['owners'])}/6</b>"),
        reply_markup=owners_menu_kb(d),
        parse_mode="HTML"
    )
...
async def owner_admins_menu(cq: CallbackQuery, state: FSMContext):
    ...
    admins = d.get("admins", [])
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_SHIELD, '🛡')} <b>Admins</b>\n\nTotal: <b>{len(admins)}</b>"),
...
async def owner_admins_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_SHIELD, '🛡')} <b>Add Admin</b>\n\nTelegram User ID bhejo:"),
...
async def owner_admins_add_done(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Valid ID bhejo."), parse_mode="HTML")
        return
    if new_id in d.get("admins", []) or is_owner(new_id, d):
        await state.clear()
        await msg.answer(sc_html(f"{em(EMOJI_WARNING, '⚠️')} Already admin/owner!"), reply_markup=admins_menu_kb(d), parse_mode="HTML")
        return
    d["admins"].append(new_id)
    save(d)
    await state.clear()
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} <b>Admin Added!</b>\n<code>{new_id}</code>"),
        reply_markup=admins_menu_kb(load()),
        parse_mode="HTML"
    )
    try:
        await msg.bot.send_message(new_id, sc_html(f"{em(EMOJI_SHIELD, '🛡')} <b>Aapko Admin bana diya gaya!</b>\n/start karein."), parse_mode="HTML")
    except: pass
...
async def owner_free_toggle(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.answer(sc_html(f"Done! {mode}"), show_alert=True)
    try:
        await cq.message.edit_text(sc_html(card(owner_panel_text(d))), reply_markup=owner_kb(d, uid), parse_mode="HTML")
...
async def panel_users_list(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(sc_html(text), reply_markup=markup, parse_mode="HTML")
...
async def panel_ban_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_CROSS, '🚫')} <b>Ban User</b>\n\nUser ka Telegram ID bhejo:"),
...
async def panel_ban_done(msg: Message, state: FSMContext):
    ...
    if is_privileged_target(ban_id, d):
        await state.clear()
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Owner/Admin/Reseller ko ban nahi kar sakte!"), parse_mode="HTML")
        return
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_CROSS, '🚫')} <b>Ban ho gaya!</b>\n<code>{ban_id}</code>"),
        reply_markup=back_kb,
        parse_mode="HTML"
    )
    try:
        await msg.bot.send_message(ban_id, sc_html(f"{em(EMOJI_CROSS, '🚫')} Aapko ban kar diya gaya. Admin se contact karein."), parse_mode="HTML")
    except: pass
...
async def panel_unban_menu(cq: CallbackQuery, state: FSMContext):
    ...
    banned = d.get("banned", [])
    if not banned:
        await cq.answer(sc_html("✅ Koi banned nahi!"), show_alert=True)
        return
    prefix = "owner" if is_owner(cq.from_user.id, d) else "admin"
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_CHECK, '🔓')} <b>Unban User</b>\n\nBanned: <b>{len(banned)}</b>"),
...
async def panel_unban_do(cq: CallbackQuery, state: FSMContext):
    ...
    if ban_id in d.get("banned", []):
        ...
        save(d)
    await cq.answer(sc_html(f"✅ {ban_id} unban ho gaya!"), show_alert=True)
    ...
    await cq.message.edit_text(sc_html(back_text), reply_markup=back_kb, parse_mode="HTML")
    try:
        await cq.bot.send_message(ban_id, sc_html(f"{em(EMOJI_CHECK, '✅')} Aapka ban hata diya gaya. /start karein."), parse_mode="HTML")
    except: pass
...
async def panel_broadcast_start(cq: CallbackQuery, state: FSMContext):
    ...
    back = "owner:home" if is_owner(cq.from_user.id, d) else "admin:home"
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_BELL, '📢')} <b>Broadcast</b>\n\nJo message bhejni hai woh type karo:"),
        reply_markup=kb([(f"{sc('cancel')}", back)]),
        parse_mode="HTML"
    )
...
async def panel_broadcast_do(msg: Message, state: FSMContext):
    ...
    await state.clear()
    users = d.get("users", {})
    wait = await msg.answer(sc_html(f"{em(EMOJI_BELL, '📤')} Broadcasting to <b>{len(users)}</b> users..."), parse_mode="HTML")
    ok = 0
    fail = 0
    for uid_str in users:
        try:
            ...
            if msg.text:
                ...
                await msg.bot.send_message(target, sc_html(bcast_text), parse_mode="HTML")
            else:
...
async def owner_export_script(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_owner(cq.from_user.id, d):
        await cq.answer(sc_html("🚫 Owner Only!"), show_alert=True)
        return
    await cq.answer(sc_html("📤 Exporting..."))
    try:
        ...
        await cq.message.reply_document(
            ...
            caption=sc_html(f"{em(EMOJI_GEAR, '📤')} <b>Script Export</b> — <i>{_VERSION}</i>"),
            parse_mode="HTML"
        )
    except Exception as e:
        await cq.answer(sc_html(f"❌ Export failed: {str(e)[:40]}"), show_alert=True)
...
async def admin_home(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_admin(cq.from_user.id, d):
        ...
        return
    try:
        await cq.message.edit_text(sc_html(card(admin_panel_text(d))), reply_markup=admin_kb(d), parse_mode="HTML")
    except TelegramBadRequest: pass
...
async def owner_fj_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows = [
        ...
        [InlineKeyboardButton(
            text=sc_html(f"🟢 {sc('enable')}" if not fj.get("enabled") else f"🔴 {sc('disable')}"),
            callback_data="owner:fj:toggle"
        )],
        [btn("ʙᴀᴄᴋ", "owner:home", EMOJI_GEAR, "🔙")]
    ]
    await cq.message.edit_text(sc_html(text), reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def owner_fj_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_BELL, '🔗')} <b>Add Force Join Channel</b>\n\n"
...
async def owner_fj_add_channel(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Valid channel ID bhejo (numbers only, e.g. -100xxx)."), parse_mode="HTML")
        return
    await state.update_data(fj_channel_id=ch_id)
    await state.set_state(S.fj_add_link)
    await msg.answer(
        sc_html(f"{em(EMOJI_BELL, '🔗')} <b>{sc('step 2/2')}</b>\n\n"
...
async def owner_fj_add_link(msg: Message, state: FSMContext):
    ...
    if not link.startswith("http"):
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Valid link bhejo (https:// se start hona chahiye)."), parse_mode="HTML")
        return
    ...
    await state.clear()
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} <b>Channel Added!</b>\n\n{em(EMOJI_BELL, '📢')} {title}\n{em(EMOJI_GEAR, '🔗')} {link}"),
...
async def owner_fj_remove_menu(cq: CallbackQuery, state: FSMContext):
    ...
    if not channels:
        await cq.answer(sc_html("❌ Koi channel nahi hai!"), show_alert=True)
        return
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_CROSS, '🗑')} <b>Remove Channel</b>\n\nKaunsa channel hataana hai?"),
        reply_markup=InlineKeyboardMarkup(inline_keyboard=rows),
        parse_mode="HTML"
    )
...
async def owner_fj_del(cq: CallbackQuery, state: FSMContext):
    ...
    save(d)
    await cq.answer(sc_html("🗑 Channel removed!"))
...
async def owner_fj_toggle(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.answer(sc_html(f"Force Join {status}!"), show_alert=True)
    await owner_fj_menu(cq, state)
...
async def owner_pricing_name(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_MONEY, '💳')} <b>{sc('step 2/4')}</b>\n\nPrice bhejo:\n<i>Example: 50</i>"),
        reply_markup=kb([(f"{sc('cancel')}", "owner:pricing:menu")]),
        parse_mode="HTML"
    )
...
async def owner_pricing_price(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Valid price bhejo (numbers only)."), parse_mode="HTML")
        return
    await state.set_state(S.add_plan_credits)
    await msg.answer(
        sc_html(f"{em(EMOJI_MONEY, '💳')} <b>{sc('step 3/4')}</b>\n\nKitne credits dena hai?\n<i>Example: 100</i>"),
        reply_markup=kb([(f"{sc('cancel')}", "owner:pricing:menu")]),
        parse_mode="HTML"
    )
...
async def owner_pricing_credits(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Valid number bhejo."), parse_mode="HTML")
        return
    await state.set_state(S.add_plan_link)
    await msg.answer(
        sc_html(f"{em(EMOJI_MONEY, '💳')} <b>{sc('step 4/4')}</b>\n\nPayment redirect link bhejo:\n"
...
async def owner_pricing_link(msg: Message, state: FSMContext):
    ...
    link = msg.text.strip()
    if not link.startswith("http"):
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Valid URL bhejo (https:// se start hona chahiye)."), parse_mode="HTML")
        return
...
async def owner_pricing_remove(cq: CallbackQuery, state: FSMContext):
    ...
    plans = d.get("pricing", {}).get("plans", [])
    if not plans:
        await cq.answer(sc_html("❌ Koi plan nahi hai!"), show_alert=True)
        return
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_CROSS, '🗑')} <b>Remove Plan</b>\n\nKaunsa plan hataana hai?"),
        reply_markup=InlineKeyboardMarkup(inline_keyboard=rows),
        parse_mode="HTML"
    )
...
async def owner_pricing_del(cq: CallbackQuery, state: FSMContext):
    ...
    save(d)
    await cq.answer(sc_html("🗑 Plan removed!"))
...
async def owner_redeem_gen_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_GIFT, '🎁')} <b>Generate Redeem Code</b>\n\n{sc('step 1/2')}: Kitne credits dena hai?\n<i>Example: 50</i>"),
...
async def owner_redeem_credits(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_GIFT, '🎁')} <b>{sc('step 2/2')}</b>\n\nKitni baar use ho sakta hai?\n<i>Example: 10</i>"),
        reply_markup=kb([(f"{sc('cancel')}", "owner:redeem:menu")]),
        parse_mode="HTML"
    )
...
async def owner_redeem_uses(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Valid number bhejo (1 ya zyada)."), parse_mode="HTML")
        return
    ...
    await state.clear()
    await msg.answer(
        sc_html(f"{em(EMOJI_GIFT, '🎉')} <b>Redeem Code Generated!</b>\n\n"
...
async def owner_redeem_del_menu(cq: CallbackQuery, state: FSMContext):
    ...
    codes = d.get("redeem_codes", {})
    if not codes:
        await cq.answer(sc_html("❌ Koi code nahi hai!"), show_alert=True)
        return
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_CROSS, '🗑')} <b>Delete Redeem Code</b>"),
        reply_markup=InlineKeyboardMarkup(inline_keyboard=rows),
        parse_mode="HTML"
    )
...
async def owner_redeem_del_do(cq: CallbackQuery, state: FSMContext):
    ...
    if code in d.get("redeem_codes", {}):
        ...
        save(d)
    await cq.answer(sc_html("🗑 Code deleted!"))
...
async def owner_credits_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_MONEY, '💰')} <b>Add Credits</b>\n\n{sc('step 1/2')}: User ka Telegram ID bhejo:"),
        reply_markup=kb([(f"{sc('cancel')}", "owner:home")]),
        parse_mode="HTML"
    )
...
async def owner_credits_add_uid(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_MONEY, '💰')} <b>{sc('step 2/2')}</b>\n\nKitne credits add karne hain?"),
        reply_markup=kb([(f"{sc('cancel')}", "owner:home")]),
        parse_mode="HTML"
    )
...
async def owner_credits_deduct_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_MONEY, '💰')} <b>Deduct Credits</b>\n\n{sc('step 1/2')}: User ka Telegram ID bhejo:"),
        reply_markup=kb([(f"{sc('cancel')}", "owner:home")]),
        parse_mode="HTML"
    )
...
async def owner_credits_deduct_amount(msg: Message, state: FSMContext):
    ...
    if success:
        try:
            await msg.bot.send_message(
                uid,
                sc_html(f"{em(EMOJI_WARNING, '⚠️')} <b>Credits Deducted!</b>\n\n-{amount} credits kat gaye.\n{em(EMOJI_MONEY, '💳')} Balance: <b>{get_user_credits(uid, d)}</b>"),
                parse_mode="HTML"
            )
        except: pass
        await msg.answer(
            sc_html(f"{em(EMOJI_CHECK, '✅')} <b>{amount} credits</b> deducted from <code>{uid}</code>!\n{em(EMOJI_MONEY, '💳')} New Balance: <b>{get_user_credits(uid, d)}</b>"),
            reply_markup=kb([(f"{sc('back')}", "owner:home")]),
            parse_mode="HTML"
        )
    else:
        await msg.answer(
            sc_html(f"{em(EMOJI_CROSS, '❌')} Insufficient credits! User ke paas sirf <b>{get_user_credits(uid, d)}</b> credits hain."),
            reply_markup=kb([(f"{sc('back')}", "owner:home")]),
            parse_mode="HTML"
        )
...
async def reseller_home(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_reseller(uid, d):
        await cq.answer(sc_html("🚫 Access Denied!"), show_alert=True)
        return
    await cq.message.edit_text(sc_html(card(reseller_panel_text(uid, d))), reply_markup=reseller_kb(d), parse_mode="HTML")
...
async def reseller_balance_view(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_reseller(uid, d):
        await cq.answer(sc_html("🚫 Access Denied!"), show_alert=True); return
    await cq.answer(sc_html(f"💰 Reseller Balance: {reseller_balance(uid, d)}\n💳 Personal Credits: {get_user_credits(uid, d)}"), show_alert=True)
...
async def reseller_got_message(msg: Message, state: FSMContext):
    ...
    await msg.answer(sc_html(f"{em(EMOJI_ROCKET, '⚡')} <b>{sc('step 3/4')} — {sc('speed')}</b>"), reply_markup=speed_kb("reseller"), parse_mode="HTML")
...
async def reseller_speed_selected(cq: CallbackQuery, state: FSMContext):
    d = load(); uid = cq.from_user.id
    if not is_reseller(uid, d): await cq.answer(sc_html("🚫 Access Denied!"), show_alert=True); return
...
async def reseller_got_count(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(sc_html("❌ Sirf positive number bhejo."), parse_mode="HTML"); return
    credits = get_user_credits(uid, d)
    if credits <= 0:
        await state.clear(); await msg.answer(sc_html("❌ Aapke paas personal credits nahi hain."), reply_markup=kb([(f"{sc('home')}", "reseller:home")]), parse_mode="HTML"); return
    if count > credits: count = credits
    await state.clear()
    devices = get_cached_devices() or await get_all_online_devices(d)
    if not devices:
        await msg.answer(sc_html("😴 Koi API online nahi! Try later."), reply_markup=kb([(f"{sc('home')}", "reseller:home")]), parse_mode="HTML"); return
...
async def reseller_unban_menu(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_reseller(uid, d): await cq.answer(sc_html("🚫 Access Denied!"), show_alert=True); return
    banned = d.get("banned", [])
    if not banned: await cq.answer(sc_html("✅ Koi banned nahi!"), show_alert=True); return
    await cq.message.edit_text(sc_html(f"🔓 <b>Unban User</b>\n\nBanned: <b>{len(banned)}</b>"), reply_markup=unban_menu_kb(d, "reseller"), parse_mode="HTML")
...
async def reseller_unban_do(cq: CallbackQuery, state: FSMContext):
    ...
    if is_privileged_target(target, d):
        await cq.answer(sc_html("❌ Owner/Admin/Reseller ko unban nahi kar sakte!"), show_alert=True); return
    if target in d.get("banned", []): d["banned"].remove(target); save(d)
    await cq.answer(sc_html(f"✅ {target} unban ho gaya!"), show_alert=True)
    await cq.message.edit_text(sc_html(card(reseller_panel_text(uid, d))), reply_markup=reseller_kb(d), parse_mode="HTML")
...
async def reseller_add_credit_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(sc_html(f"💰 <b>Add Credit</b>\n\nReseller Balance: <b>{reseller_balance(uid, d)}</b>\n\nNormal User ka Telegram ID bhejo:"), reply_markup=kb([(f"{sc('cancel')}", "reseller:home")]), parse_mode="HTML")
...
async def reseller_add_credit_uid(msg: Message, state: FSMContext):
    ...
    except: await msg.answer(sc_html("❌ Valid User ID bhejo.")); return
    if is_privileged_target(target, d): await state.clear(); await msg.answer(sc_html("❌ Sirf normal users ko credits de sakte ho.")); return
    if str(target) not in d.get("users", {}): await state.clear(); await msg.answer(sc_html("❌ User ID exist nahi karta.")); return
    await state.update_data(reseller_target=target)
    await state.set_state(S.reseller_add_credit_amount)
    await msg.answer(sc_html(f"💰 Balance: <b>{reseller_balance(uid, d)}</b>\nKitne credits add karne hain?"), reply_markup=kb([(f"{sc('cancel')}", "reseller:home")]), parse_mode="HTML")
...
async def reseller_add_credit_amount(msg: Message, state: FSMContext):
    ...
    except: await msg.answer(sc_html("❌ Positive amount bhejo.")); return
    data = await state.get_data(); target = int(data.get("reseller_target", 0))
    if is_privileged_target(target, d): await state.clear(); await msg.answer(sc_html("❌ Sirf normal users ko credits de sakte ho.")); return
    bal = reseller_balance(uid, d)
    if amount > bal:
        await state.clear(); await msg.answer(sc_html(f"❌ Insufficient reseller balance! Available: <b>{bal}</b>"), parse_mode="HTML"); return
...
async def reseller_deduct_credit_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(sc_html("💰 <b>Deduct Credit</b>\n\nNormal User ka Telegram ID bhejo:"), reply_markup=kb([(f"{sc('cancel')}", "reseller:home")]), parse_mode="HTML")
...
async def reseller_deduct_credit_uid(msg: Message, state: FSMContext):
    ...
    if is_privileged_target(target, d): await state.clear(); await msg.answer(sc_html("❌ Sirf normal users ko credits deduct kar sakte ho.")); return
    if str(target) not in d.get("users", {}): await state.clear(); await msg.answer(sc_html("❌ User ID exist nahi karta.")); return
    await state.update_data(reseller_target=target); await state.set_state(S.reseller_deduct_credit_amount)
    await msg.answer(sc_html(f"User Credits: <b>{get_user_credits(target, d)}</b>\nKitne credits deduct karne hain?"), reply_markup=kb([(f"{sc('cancel')}", "reseller:home")]), parse_mode="HTML")
...
async def reseller_deduct_credit_amount(msg: Message, state: FSMContext):
    ...
    if not deduct_credits(target, amount, d):
        await state.clear(); await msg.answer(sc_html(f"❌ Insufficient user credits! Current: <b>{get_user_credits(target, d)}</b>"), parse_mode="HTML"); return
    save(d); await state.clear(); log_activity(d, "reseller_deduct_credit", uid, f"Target {target}, amount {amount}"); save(d)
    await msg.answer(sc_html(f"✅ <b>{amount} credits</b> deducted from <code>{target}</code>.\n💳 User Balance: <b>{get_user_credits(target, d)}</b>"), reply_markup=reseller_kb(d), parse_mode="HTML")
    try: await msg.bot.send_message(target, sc_html(f"⚠️ <b>Credits Deducted!</b>\n-{amount} credits\n💳 Balance: <b>{get_user_credits(target, d)}</b>"), parse_mode="HTML")
    except: pass
...
async def reseller_redeem_credits(msg: Message, state: FSMContext):
    ...
    except: await msg.answer(sc_html("❌ Positive credits bhejo.")); return
    if credits > reseller_balance(uid, d): await state.clear(); await msg.answer(sc_html(f"❌ Insufficient reseller balance! Available: <b>{reseller_balance(uid, d)}</b>"), parse_mode="HTML"); return
    await state.update_data(gen_credits=credits); await state.set_state(S.reseller_redeem_uses)
    await msg.answer(sc_html("🔢 Code kitni baar use ho sakta hai?"), reply_markup=kb([(f"{sc('cancel')}", "reseller:home")]), parse_mode="HTML")
...
async def reseller_redeem_uses(msg: Message, state: FSMContext):
    ...
    except: await msg.answer(sc_html("❌ 1 ya zyada uses bhejo.")); return
    data = await state.get_data(); credits = int(data.get("gen_credits", 0)); bal = reseller_balance(uid, d)
    if credits <= 0 or credits > bal:
        await state.clear(); await msg.answer(sc_html("❌ Insufficient reseller balance.")); return
    while True:
        code = "GIFT" + "".join(random.choices(string.ascii_uppercase + string.digits, k=8))
    ...
    await msg.answer(sc_html(f"🎉 <b>Redeem Code Generated!</b>\n\nCode: <code>{code}</code>\n💰 Credits: <b>{credits}</b>\n🔢 Max Uses: <b>{uses}</b>\n💳 Reseller Balance: <b>{reseller_balance(uid, d)}</b>"), reply_markup=reseller_kb(d), parse_mode="HTML")
...
async def reseller_protect_done(msg: Message, state: FSMContext):
    ...
    number = msg.text.strip()
    if not number.replace("+", "").replace(" ", "").isdigit() or len(number) < 7:
        await msg.answer(sc_html("❌ Invalid number.")); return
...
async def owner_settings_ref(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_GIFT, '🎁')} <b>Set Referral Credits</b>\n\nReferral pe kitne credits dena hai?\n<i>Example: 5</i>"),
...
async def owner_settings_ref_done(msg: Message, state: FSMContext):
    ...
    await state.clear()
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} <b>Referral Credits Updated!</b>\n\nAb har referral pe <b>{credits}</b> credits milenge."),
...
async def user_home(cq: CallbackQuery, state: FSMContext):
    ...
    if is_reseller(uid, d) and not is_admin(uid, d) and not is_owner(uid, d):
        await cq.message.edit_text(sc_html(card(reseller_panel_text(uid, d))), reply_markup=reseller_kb(d), parse_mode="HTML")
        return
    if not can_use(uid, d) and not is_owner(uid, d) and not is_admin(uid, d):
        await cq.message.edit_text(sc_html(f"{em(EMOJI_CROSS, '⛔')} ᴀᴄᴄᴇꜱꜱ ɴᴀʜɪ ʜᴀɪ !"), parse_mode="HTML")
        return
    try:
    ...
    if not ok:
        await cq.bot.send_message(
            uid,
            sc_html(card(user_home_text(uid, d))),
            reply_markup=user_reply_keyboard(uid, d),
            parse_mode="HTML",
        )
...
async def user_credits(cq: CallbackQuery, state: FSMContext):
    ...
    uid = cq.from_user.id
    credits = get_user_credits(uid, d)
    await cq.answer(sc_html(f"💰 Credits: {credits}\nOwner: {SUPER_ADMIN_NAME}"), show_alert=True)
...
async def user_redeem_done(msg: Message, state: FSMContext):
    ...
    if code not in codes:
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Invalid redeem code!"), reply_markup=kb([(f"{sc('home')}", "user:home")]), parse_mode="HTML")
        return

    code_data = codes[code]
    if code_data.get("uses_left", 0) <= 0:
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Ye code expire ho gaya hai!"), reply_markup=kb([(f"{sc('home')}", "user:home")]), parse_mode="HTML")
        return

    if uid in code_data.get("used_by", []):
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Aap pehle se ye code use kar chuke hain!"), reply_markup=kb([(f"{sc('home')}", "user:home")]), parse_mode="HTML")
        return
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_GIFT, '🎉')} <b>Redeem Successful!</b>\n\n{em(EMOJI_MONEY, '💰')} +{credits} credits added!\n{em(EMOJI_MONEY, '💳')} Balance: <b>{get_user_credits(uid, d)}</b>"),
        reply_markup=kb([(f"{sc('home')}", "user:home")]),
        parse_mode="HTML"
    )
...
async def user_refer(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_STAR, '👥')} <b>Referral Program</b>\n\n"
...
async def user_stats(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_STAR, '📊')} <b>Your Stats</b>\n\n"
...
async def user_sms_history(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(sc_html(text), reply_markup=kb([(f"{sc('back')}", "user:home")]), parse_mode="HTML")
...
async def user_pricing(cq: CallbackQuery, state: FSMContext):
    ...
    if not plans:
        await cq.answer(sc_html("❌ Abhi koi plan available nahi!"), show_alert=True)
        return
...
async def user_info(cq: CallbackQuery, state: FSMContext):
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_GEAR, 'ℹ️')} <b>SMS Blast Bot {_VERSION}</b>\n\n"
...
async def owner_premium_add_start(cq: CallbackQuery, state: FSMContext):
    if not is_premium_admin(cq.from_user.id):
        await cq.answer(sc_html("🚫 Super Admin Only!"), show_alert=True)
        return
    await state.set_state(S.premium_add_user)
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_MONEY, '💎')} <b>Sell Premium Credit</b>\n\n"
...
async def owner_premium_add_user(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(
            sc_html(f"{em(EMOJI_CROSS, '❌')} Valid positive User ID bhejo."),
            parse_mode="HTML"
        )
        return
...
async def owner_premium_add_amount(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(
            sc_html(f"{em(EMOJI_CROSS, '❌')} Positive integer amount bhejo."),
            parse_mode="HTML"
        )
        return
    ...
    if not isinstance(user_id, int) or user_id <= 0:
        await state.clear()
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Invalid premium user ID."), parse_mode="HTML")
        return
    ...
    try:
        ...
        await state.clear()
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Premium credit save failed."), parse_mode="HTML")
        return
    ...
    if str(user_id) in d.get("users", {}):
        try:
            await msg.bot.send_message(
                user_id,
                sc_html(f"{em(EMOJI_MONEY, '💎')} <b>Premium Credits Added!</b>\n\n"
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} <b>Premium Credits Added!</b>\n\n"
...
async def owner_premium_export(cq: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await cq.message.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Export failed."), parse_mode="HTML")
...
async def owner_premium_import_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        sc_html(f"{em(EMOJI_GEAR, '📥')} <b>Import Premium Credits</b>\n\n"
        f".json file upload karo.\n"
        f'Format: <code>{{"USERID": CREDIT}}</code>\n'
        f'Example: <code>{{"8834661642": 999999776}}</code>\n\n'
...
async def owner_premium_import_file(msg: Message, state: FSMContext):
    ...
    if not msg.document:
        await msg.answer(
            sc_html(f"{em(EMOJI_CROSS, '❌')} Please .txt document upload karo."),
            parse_mode="HTML"
        )
        return

    filename = msg.document.file_name or ""
    if filename != "ᴩʀɪʏᴀɴꜱʜᴜ.json":
        await msg.answer(
            sc_html(f"{em(EMOJI_CROSS, '❌')} Sirf <code>ᴩʀɪʏᴀɴꜱʜᴜ.json</code> file allowed hai."),
            parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        await state.clear()
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} File read nahi ho saki."), parse_mode="HTML")
        return
    ...
    try:
        ...
        for uid_text, credit in payload.items():
            try:
        ...
        await state.clear()
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Invalid ᴩʀɪʏᴀɴꜱʜᴜ.json format."), parse_mode="HTML")
        return
    ...
    try:
        ...
        await state.clear()
        await msg.answer(sc_html(f"{em(EMOJI_CROSS, '❌')} Import save failed."), parse_mode="HTML")
        return
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_CHECK, '✅')} <b>Premium Database Imported!</b>\n\n"
...
# ===== Reply keyboard shortcuts (colored bottom menu) =====
@R.message(F.text.in_({"sᴛᴀʀᴛ ʙᴏᴍʙɪɴɢ", "Start Bombing", "🚀 sᴛᴀʀᴛ ʙᴏᴍʙɪɴɢ", "🚀 Start Bombing"}))
async def rk_start_bombing(msg: Message, state: FSMContext):
    class _CQ:
        ...
        async def answer(self, *a, **k):
            return None
    # reuse callback flow via editing-style: trigger same state entry
    from_user = msg.from_user
    d = load()
    uid = from_user.id
    if is_banned(uid, d):
        await msg.answer(sc_html("Banned."))
        return
    joined, missing = await user_joined_all(msg.bot, uid, d)
    if not joined:
        await msg.answer(sc_html(force_join_text(missing)), reply_markup=force_join_kb(missing), parse_mode="HTML", disable_web_page_preview=True)
    ...
    await state.set_state(S.send_number)
    d = load()
    credits = get_user_credits(msg.from_user.id, d)
    await msg.answer(
        sc_html(card(number_step_text(credits))),
        reply_markup=kb([(f"{sc('cancel')}", "user:home")]),
        parse_mode="HTML"
...
@R.message(F.text.in_({"ᴠɪᴅᴇᴏs", "Videos", "📹 ᴠɪᴅᴇᴏs", "📹 Videos"}))
async def rk_videos(msg: Message, state: FSMContext):
    await state.clear()
    d = load()
    await send_random_video(msg.bot, msg.chat.id, caption=f"{em(EMOJI_VIDEO, '📹')} Random Video")


@R.message(F.text.in_({"ᴀᴄᴄᴏᴜɴᴛ & ʙᴀʟᴀɴᴄᴇ", "Account & Balance", "💰 ᴀᴄᴄᴏᴜɴᴛ & ʙᴀʟᴀɴᴄᴇ", "💰 Account & Balance"}))
async def rk_balance(msg: Message, state: FSMContext):
    d = load()
    credits = get_user_credits(msg.from_user.id, d)
    await msg.answer(sc_html(card(f"{em(EMOJI_MONEY, '💰')} Credits: <b>{credits}</b>\nOwner: {SUPER_ADMIN_NAME}")), parse_mode="HTML")


@R.message(F.text.in_({"ʀᴇᴅᴇᴇᴍ", "Redeem", "🎁 ʀᴇᴅᴇᴇᴍ", "🎁 Redeem"}))
...
@R.message(F.text.in_({"ʀᴇғᴇʀ & ᴇᴀʀɴ", "Refer & Earn", "👥 ʀᴇғᴇʀ & ᴇᴀʀɴ", "👥 Refer & Earn"}))
async def rk_refer(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        sc_html(f"{em(EMOJI_STAR, '👥')} <b>Refer & Earn</b>\n\n"
        f"Code: <code>{code}</code>\n"
        f"Link: https://t.me/{me.username}?start={code}\n"
        f"Reward: <b>{ref_credits}</b> credits"),
        parse_mode="HTML"
    )


@R.message(F.text.in_({"sᴛᴀᴛs", "Stats", "📊 sᴛᴀᴛs", "📊 Stats"}))
async def rk_stats(msg: Message, state: FSMContext):
    d = load()
    uid = msg.from_user.id
    udata = d["users"].get(str(uid), {})
    stats = d.get("stats", {})
...
@R.message(F.text.in_({"ᴍʏ sᴍs ʜɪsᴛᴏʀʏ", "My SMS History", "📜 ᴍʏ sᴍs ʜɪsᴛᴏʀʏ", "📜 My SMS History"}))
async def rk_history(msg: Message, state: FSMContext):
    d = load()
    uid = msg.from_user.id
    history = d.get("sms_history", {}).get(str(uid), [])[-10:]
    if not history:
        await msg.answer(sc_html(f"{em(EMOJI_GEAR, '📜')} Abhi koi SMS history nahi."), parse_mode="HTML")
        return
    lines = [f"{em(EMOJI_GEAR, '📜')} <b>My SMS History</b>\n"]
    for i, entry in enumerate(reversed(history), 1):
        ts = fmt_time(entry.get("timestamp", 0))
        num = entry.get("number", "?")
        lines.append(f"{i}. [{ts}] <code>{mask_number(num)}</code>")
    await msg.answer(sc_html("\n".join(lines)), parse_mode="HTML")


@R.message(F.text.in_({"ʙᴜʏ ᴄʀᴇᴅɪᴛs", "Buy Credits", "💳 ʙᴜʏ ᴄʀᴇᴅɪᴛs", "💳 Buy Credits"}))
async def rk_buy(msg: Message, state: FSMContext):
    d = load()
    plans = d.get("pricing", {}).get("plans", [])
    if not plans:
        await msg.answer(sc_html("❌ Abhi koi plan available nahi!"))
        return
    text = f"{em(EMOJI_MONEY, '💰')} <b>Buy Credits</b>\n\n"
    rows = []
    for plan in plans:
        text += f"• <b>{plan['name']}</b> — {plan['price']} {plan.get('currency', 'INR')} — {plan['credits']} credits\n"
        rows.append([btn_url(f"Buy {sc(plan['name'][:20])}", plan['payment_link'], EMOJI_MONEY, "💳")])
    await msg.answer(sc_html(text), reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")


@R.message(F.text.in_({"sᴛᴏᴘ ʙᴏᴍʙɪɴɢ", "Stop Bombing", "🛑 sᴛᴏᴘ ʙᴏᴍʙɪɴɢ", "🛑 Stop Bombing", "🛑 sᴛᴏᴘ ʙʟᴀsᴛ", "🛑 Stop Blast"}))
async def rk_stop(msg: Message, state: FSMContext):
    uid = msg.from_user.id
    async with SESSIONS_LOCK:
        sess = USER_SESSIONS.get(uid)
        if sess:
            sess.cancelled = True
            await msg.answer(sc_html(f"{em(EMOJI_CHECK, '🛑')} Blast stop request sent."), parse_mode="HTML")
        else:
            await msg.answer(sc_html("Koi active blast nahi hai."))


@R.message(F.text.in_({"ᴀᴅᴍɪɴ ᴘᴀɴᴇʟ", "Admin Panel", "⚙️ ᴀᴅᴍɪɴ ᴘᴀɴᴇʟ", "⚙️ Admin Panel"}))
async def rk_admin(msg: Message, state: FSMContext):
    d = load()
    uid = msg.from_user.id
    if is_owner(uid, d):
        await msg.answer(sc_html(card(owner_panel_text(d))), reply_markup=owner_kb(d, uid), parse_mode="HTML")
    elif is_admin(uid, d):
        await msg.answer(sc_html(card(admin_panel_text(d))), reply_markup=admin_kb(d), parse_mode="HTML")
    else:
        await msg.answer(sc_html("🚫 Admin only!"))
...
async def main():
    ...
    try:
        await bot.send_message(
            MAIN_OWNER,
            sc_html(f"{em(EMOJI_ROCKET, '🚀')} <b>SMS Blast Bot {_VERSION} Online!</b>\n@{me.username}\n"
