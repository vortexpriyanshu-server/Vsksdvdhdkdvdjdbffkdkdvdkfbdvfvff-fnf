...
_PREMIUM_FILE = os.path.join(os.path.dirname(os.path.abspath(_DATA_FILE)), "priyanshu.txt")
...
async def load_premium_db() -> dict[int, int]:
    """Load the independent premium-credit database from priyanshu.txt."""
            ...
            with open(_PREMIUM_FILE, "r", encoding="utf-8") as f:
                for raw_line in f:
                    ...
                    if not line or "=" not in line:
                        continue
                    uid_text, credit_text = line.split("=", 1)
                    try:
                        uid = int(uid_text.strip())
                        credit = int(credit_text.strip())
...
async def save_premium_db(data: dict[int, int]):
    """Atomically rewrite the independent premium-credit database."""
    async with _PREMIUM_DB_LOCK:
        try:
            ...
            with open(tmp_file, "w", encoding="utf-8") as f:
                for uid in sorted(data):
                    f.write(f"{int(uid)}={int(data[uid])}\n")
...
async def _send_premium_export(bot: Bot, chat_id: int, empty_text: str = "No premium credits data found."):
    """Send the current priyanshu.txt as a Telegram document."""
    async with _PREMIUM_DB_LOCK:
        if not os.path.exists(_PREMIUM_FILE):
            content = b""
        else:
    ...
    if str(user_id) in d.get("users", {}):
        try:
            await msg.bot.send_message(
                ...
                f"+{amount} premium credits mile hain.\n"
                f"{em(EMOJI_MONEY, '💳')} Premium Balance: <b>{new_balance}</b>",
                parse_mode="HTML"
            )
        except Exception:
            pass
...
async def owner_premium_import_file(msg: Message, state: FSMContext):
    ...
    if not filename.endswith(".txt"):
        await msg.answer(
            f"{em(EMOJI_CROSS, '❌')} Sirf .txt file allowed hai.",
            parse_mode="HTML"
        )
        return
