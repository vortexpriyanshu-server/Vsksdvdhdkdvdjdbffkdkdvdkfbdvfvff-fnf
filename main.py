...
def default_reply_keyboard() -> ReplyKeyboardMarkup:
    """Luffy-style bottom Reply Keyboard shown above Telegram's typing bar."""
    return ReplyKeyboardMarkup(
        keyboard=[
            [style_btn("📤 SEND SMS", style="primary"), style_btn("📹 VIDEOS", style="primary")],
            [style_btn("💳 CREDITS", style="primary"), style_btn("👥 REFER & EARN", style="primary")],
            [style_btn("🎁 REDEEM CODE", style="primary"), style_btn("📊 MY STATS", style="primary")],
            [style_btn("📜 SMS HISTORY", style="primary"), style_btn("💎 PREMIUM ACCESS", style="success")],
            [style_btn("ℹ️ BOT INFO", style="primary"), style_btn("🏠 HOME", style="primary")],
        ],
...
def role_reply_keyboard(uid: int, d: dict) -> ReplyKeyboardMarkup:
    """Bottom keyboard for every role, with a role-specific panel button."""
    rows = [
        [style_btn("📤 SEND SMS", style="primary"), style_btn("📹 VIDEOS", style="primary")],
        [style_btn("💳 CREDITS", style="primary"), style_btn("👥 REFER & EARN", style="primary")],
        [style_btn("🎁 REDEEM CODE", style="primary"), style_btn("📊 MY STATS", style="primary")],
    ...
    if is_owner(uid, d):
        ...
        rows.append([style_btn("💼 RESELLER PANEL", style="danger")])
...
# Starting image from the user's Telegram post. Existing random-video behaviour
# remains intact; this image is simply added before it.
START_IMAGE_CHAT = "@priyanshurer"
START_IMAGE_MESSAGE_ID = 2

async def send_start_image(bot: Bot, chat_id: int):
    try:
        await bot.copy_message(chat_id=chat_id, from_chat_id=START_IMAGE_CHAT, message_id=START_IMAGE_MESSAGE_ID)
    except Exception as e:
        log.warning(f"Starting image copy failed: {e}")
...
def is_owner(uid: int, d: dict) -> bool:
    # Keep the permission and UI role checks identical.  Main owner,
    # configured owners and super-admins are all treated as owners.
    return is_main_owner(uid) or uid in d.get("owners", []) or uid in SUPER_ADMINS
...
async def cmd_start_deep(msg: Message, state: FSMContext):
    ...
    await send_start_image(msg.bot, msg.chat.id)
    ...
    if is_owner(uid, d):
        await msg.answer(card(owner_panel_text(d)), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
        return
    if is_admin(uid, d):
        await msg.answer(card(admin_panel_text(d)), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
        return
    if is_reseller(uid, d):
        await msg.answer(card(reseller_panel_text(uid, d)), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
        return
    if is_banned(uid, d):
    ...
    await msg.answer(card(user_home_text(uid, d)), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
...
async def fj_check(cq: CallbackQuery, state: FSMContext):
    ...
    await send_start_image(cq.bot, cq.message.chat.id)
    await send_random_video(cq.bot, cq.message.chat.id, caption=f"{em(EMOJI_ROCKET, '🚀')} Welcome! Verified Successfully.\nOwner: {SUPER_ADMIN_NAME}")

    if is_owner(uid, d):
        await cq.message.answer(card(owner_panel_text(d)), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
    elif is_admin(uid, d):
        await cq.message.answer(card(admin_panel_text(d)), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
    elif is_reseller(uid, d):
        await cq.message.answer(card(reseller_panel_text(uid, d)), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
    else:
        await cq.message.answer(card(user_home_text(uid, d)), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
...
async def owner_free_toggle(cq: CallbackQuery, state: FSMContext):
    ...
    try:
        await cq.message.edit_text(card(owner_panel_text(d)), reply_markup=owner_kb(d, cq.from_user.id), parse_mode="HTML")
...
async def admin_home(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_admin(cq.from_user.id, d):
        ...
        return
    try:
        await cq.message.edit_text(card(admin_panel_text(d)), reply_markup=role_reply_keyboard(cq.from_user.id, d), parse_mode="HTML")
    except TelegramBadRequest: pass
...
async def reseller_home(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_reseller(uid, d):
        await cq.answer("🚫 Access Denied!", show_alert=True)
        return
    await cq.message.edit_text(card(reseller_panel_text(uid, d)), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
...
async def user_home(cq: CallbackQuery, state: FSMContext):
    ...
    if is_owner(uid, d):
        await cq.message.edit_text(card(owner_panel_text(d)), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
        return
    if is_admin(uid, d):
        await cq.message.edit_text(card(admin_panel_text(d)), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
        return
    if is_reseller(uid, d):
        await cq.message.edit_text(card(reseller_panel_text(uid, d)), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
        return
    if not can_use(uid, d):
        ...
        return
    await cq.message.edit_text(card(user_home_text(uid, d)), parse_mode="HTML")
...
async def owner_premium_import_file(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await state.clear()
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Invalid PRIYANSHU.json format.", parse_mode="HTML")
        return
...
@R.message(F.text.in_({
    "📤 sᴇɴᴅ sᴍs", "📤 SEND SMS",
    "📹 ᴠɪᴅᴇᴏs", "📹 VIDEOS",
    "💳 ᴄʀᴇᴅɪᴛs", "💳 CREDITS",
    "👥 ʀᴇғᴇʀ & ᴇᴀʀɴ", "👥 REFER & EARN",
    "🎁 ʀᴇᴅᴇᴇᴍ ᴄᴏᴅᴇ", "🎁 REDEEM CODE",
    "📊 ᴍʏ sᴛᴀᴛs", "📊 MY STATS",
    "📜 sᴍs ʜɪsᴛᴏʀʏ", "📜 SMS HISTORY",
    "💎 ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss", "💎 PREMIUM ACCESS",
    "ℹ️ ʙᴏᴛ ɪɴғᴏ", "ℹ️ BOT INFO",
    "🏠 ʜᴏᴍᴇ", "🏠 HOME",
    "🛑 sᴛᴏᴘ ʙʟᴀsᴛ", "🛑 STOP BLAST",
    "👑 ᴏᴡɴᴇʀ ᴘᴀɴᴇʟ", "👑 OWNER PANEL",
    "🛡 ᴀᴅᴍɪɴ ᴘᴀɴᴇʟ", "🛡 ADMIN PANEL",
    "💼 ʀᴇsᴇʟʟᴇʀ ᴘᴀɴᴇʟ", "💼 RESELLER PANEL"
}))
async def bottom_keyboard_router(msg: Message, state: FSMContext):
    """Handle the visible bottom keyboard without changing existing callback features."""
    uid = msg.from_user.id
    d = load()
    # Keep Telegram ReplyKeyboard text exact. Calling .upper() corrupts
    # Unicode small-cap letters used by sc(), e.g. ʀ -> Ʀ, so the
    # visible keyboard text no longer matches the router conditions.
    text = (msg.text or "").strip()

    # Do not steal messages while an existing FSM flow is waiting for input.
    current_state = await state.get_state()
    if current_state:
        return

    if text in {"📤 SEND SMS", "📤 Sᴇɴᴅ Sᴍs"}:
        joined, missing = await user_joined_all(msg.bot, uid, d)
        if not joined:
            await msg.answer(force_join_text(missing), reply_markup=force_join_kb(missing), parse_mode="HTML", disable_web_page_preview=True)
            return
        if not can_use(uid, d):
            await msg.answer(f"🚫 <b>Access denied!</b>", parse_mode="HTML")
            return
        await state.set_state(S.send_number)
        await msg.answer(
            f"{em(EMOJI_PHONE, '📞')} <b>{sc('step 1/4')} — {sc('number')}</b>\n\n"
    ...
    if text in {"📹 VIDEOS", "📹 ᴠɪᴅᴇᴏs"}:
        await send_random_video(msg.bot, msg.chat.id)
        return

    if text in {"💳 CREDITS", "💳 ᴄʀᴇᴅɪᴛs"}:
        await msg.answer(f"{em(EMOJI_MONEY, '💳')} <b>Credits:</b> {get_user_credits(uid, d)}", parse_mode="HTML")
        return

    if text in {"👥 REFER & EARN", "👥 ʀᴇғᴇʀ & ᴇᴀʀɴ"}:
        code = generate_user_refer_code(uid, d)
        save(d)
        ref_credits = d.get("settings", {}).get("ref_credits", 3)
        ...
        await msg.answer(
            ...
            f"Apna referral code share karein aur har successful referral pe <b>{ref_credits}</b> credits paayein!\n\n"
            f"{em(EMOJI_GIFT, '🎁')} Your Code: <code>{code}</code>\n\n"
            f"{em(EMOJI_GEAR, '🔗')} Share Link: https://t.me/{me.username}?start={code}",
            parse_mode="HTML", disable_web_page_preview=True
        )
        return

    if text in {"🎁 REDEEM CODE", "🎁 ʀᴇᴅᴇᴇᴍ ᴄᴏᴅᴇ"}:
        await state.set_state(S.redeem_code)
        await msg.answer(
            f"{em(EMOJI_GIFT, '🎁')} <b>Redeem Code</b>\n\nApna redeem code enter karein:\n<i>Example: GIFTABC123</i>",
            parse_mode="HTML"
    ...
    if text in {"📜 SMS HISTORY", "📜 sᴍs ʜɪsᴛᴏʀʏ"}:
        history = d.get("sms_history", {}).get(str(uid), [])[-10:]
        if not history:
            await msg.answer(f"{em(EMOJI_GEAR, '📜')} <b>Your SMS History</b>\n\n<i>Abhi tak koi SMS send nahi kiya.</i>", parse_mode="HTML")
            return
        lines = [f"{em(EMOJI_GEAR, '📜')} <b>Your SMS History</b> (Last 10)\n"]
        for i, entry in enumerate(reversed(history), 1):
            ts = fmt_time(entry.get("timestamp", 0))
    ...
    if text in {"💎 PREMIUM ACCESS", "💎 ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss"}:
        plans = d.get("pricing", {}).get("plans", [])
        if not plans:
            await msg.answer("❌ Abhi koi premium plan available nahi!", parse_mode="HTML")
            return
        lines = [f"{em(EMOJI_MONEY, '💎')} <b>Premium Access</b>\n"]
        for plan in plans:
            lines.append(f"{em(EMOJI_STAR, '📋')} <b>{plan['name']}</b> — {plan['price']} {plan.get('currency', 'INR')} = {plan['credits']} credits")
        await msg.answer("\n".join(lines), parse_mode="HTML")
        return

    if text in {"ℹ️ BOT INFO", "ℹ️ ʙᴏᴛ ɪɴғᴏ"}:
        await msg.answer(
            f"{em(EMOJI_GEAR, 'ℹ️')} <b>SMS Blast Bot {_VERSION}</b>\n\n"
            f"{em(EMOJI_GEAR, '🤖')} Bot for sending bulk SMS via Firebase-connected Android devices.\n\n"
            f"{em(EMOJI_CROWN, '👤')} Developer: <a href='{SUPER_ADMIN_LINK}'>{SUPER_ADMIN_NAME}</a>",
    ...
    if text in {"🛑 STOP BLAST", "🛑 sᴛᴏᴘ ʙʟᴀsᴛ"}:
        async with SESSIONS_LOCK:
            session = USER_SESSIONS.get(uid)
            if session and not (session.task and session.task.done()):
                session.cancelled = True
                await msg.answer("🛑 Stop signal bhej diya! Sending ruk rahi hai.")
            else:
                await msg.answer("✅ Koi active sending nahi hai.")
        return

    if text in {"🏠 HOME", "🏠 ʜᴏᴍᴇ"}:
        if is_owner(uid, d):
            panel = owner_panel_text(d)
        elif is_admin(uid, d):
            panel = admin_panel_text(d)
        elif is_reseller(uid, d):
            panel = reseller_panel_text(uid, d)
        else:
            panel = user_home_text(uid, d)
        await msg.answer(card(panel), reply_markup=role_reply_keyboard(uid, d), parse_mode="HTML")
        return

    if text in {"👑 OWNER PANEL", "👑 ᴏᴡɴᴇʀ ᴘᴀɴᴇʟ"}:
        if not is_owner(uid, d):
            return
        await msg.answer(card(owner_panel_text(d)), reply_markup=owner_kb(d, uid), parse_mode="HTML")
        return

    if text in {"🛡 ADMIN PANEL", "🛡 ᴀᴅᴍɪɴ ᴘᴀɴᴇʟ"}:
        if not is_admin(uid, d):
            return
        await msg.answer(card(admin_panel_text(d)), reply_markup=admin_kb(d), parse_mode="HTML")
        return

    if text in {"💼 RESELLER PANEL", "💼 ʀᴇsᴇʟʟᴇʀ ᴘᴀɴᴇʟ"}:
        if not is_reseller(uid, d):
            return
