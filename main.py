...
# ========== PREMIUM DATABASE SETTINGS ==========
PREMIUM_DB_FILE = "priyanshu.txt"
PREMIUM_DB_LOCK = asyncio.Lock()
...
# ==================== PREMIUM DATABASE HELPERS ====================

async def load_premium_db() -> dict:
    """Load premium credits from priyanshu.txt. Returns dict[int, int]."""
    async with PREMIUM_DB_LOCK:
        data = {}
                ...
                with open(PREMIUM_DB_FILE, "r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        if "=" in line:
                            ...
                            if user_id_str.isdigit() and credit_str.lstrip("-").isdigit():
                                data[int(user_id_str)] = int(credit_str)
            except Exception as e:
                log.error(f"Error loading premium DB: {e}")
        return data

async def save_premium_db(data: dict):
    """Save premium credits to priyanshu.txt."""
    async with PREMIUM_DB_LOCK:
        try:
            with open(PREMIUM_DB_FILE, "w", encoding="utf-8") as f:
                for uid, credits in data.items():
                    f.write(f"{uid}={credits}\n")
        except Exception as e:
            log.error(f"Error saving premium DB: {e}")

async def add_premium_credit(user_id: int, amount: int) -> bool:
    """Add premium credits to user. Returns True if success."""
    if amount <= 0:
        return False
...
async def set_premium_credit(user_id: int, amount: int):
    """Set premium credits to exact amount (for import)."""
    if amount < 0:
        amount = 0
    data = await load_premium_db()
    data[user_id] = amount
...
async def export_premium_file(bot: Bot, chat_id: int):
    """Send priyanshu.txt as a document to chat_id."""
    if os.path.exists(PREMIUM_DB_FILE):
        try:
            await bot.send_document(chat_id, document=FSInputFile(PREMIUM_DB_FILE), caption=f"{em(EMOJI_GEAR, '📤')} <b>Premium Credits Export</b>", parse_mode="HTML")
        except Exception as e:
            log.error(f"Export failed: {e}")
            await bot.send_message(chat_id, f"{em(EMOJI_CROSS, '❌')} Export failed: {e}", parse_mode="HTML")
    else:
        await bot.send_message(chat_id, f"{em(EMOJI_WARNING, '⚠️')} No premium credits data found.", parse_mode="HTML")

async def background_premium_exporter(bot: Bot):
    """Background task that exports priyanshu.txt every night at 01:01 AM."""
    log.info("Premium exporter background task started")
    while True:
        now = datetime.now()
        # Calculate next 01:01 AM
        next_run = now.replace(hour=1, minute=1, second=0, microsecond=0)
        if now >= next_run:
            # If already past 01:01 today, schedule for tomorrow
            next_run = next_run + timedelta(days=1)
        wait_seconds = (next_run - now).total_seconds()
        log.info(f"Premium exporter sleeping for {wait_seconds:.0f} seconds until {next_run}")
        await asyncio.sleep(wait_seconds)

        # Export
        try:
            await export_premium_file(bot, MAIN_OWNER)
            log.info("Premium export sent at 01:01 AM")
        except Exception as e:
            log.error(f"Premium export error: {e}")
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        # NEW PREMIUM CREDIT SYSTEM BUTTONS (only for Super Admin – handlers will check)
...
async def panel_ban_done(msg: Message, state: FSMContext):
    ...
    if ban_id not in d.get("banned", []):
        ...
        save(d)
    await state.clear()
    back_kb = owner_kb(d) if is_owner(uid, d) else reseller_kb(d) if reseller_action else admin_kb(d)
    await msg.answer(
...
# ==================== PREMIUM CREDIT SYSTEM HANDLERS ====================

@R.callback_query(F.data == "owner:premium:add")
async def premium_add_start(cq: CallbackQuery, state: FSMContext):
    if not is_main_owner(cq.from_user.id):
        await cq.answer("🚫 Only Main Owner can use this!", show_alert=True)
        return
    await state.set_state(S.premium_add_user)
    await cq.message.edit_text(
        f"{em(EMOJI_MONEY, '💎')} <b>Add Premium Credit</b>\n\n"
        f"Enter the Telegram User ID to give premium credits:\n"
...
async def premium_add_user(msg: Message, state: FSMContext):
    if not is_main_owner(msg.from_user.id):
        await state.clear()
        return
    try:
        uid = int(msg.text.strip())
    except:
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Please send a valid numeric User ID.", parse_mode="HTML")
        return
    await state.update_data(premium_target_uid=uid)
    await state.set_state(S.premium_add_amount)
    await msg.answer(
        f"{em(EMOJI_MONEY, '💎')} <b>Enter Amount</b>\n\n"
        f"How many premium credits to give to <code>{uid}</code>?\n"
        f"<i>Positive integer only.</i>",
        reply_markup=kb([(f"{sc('cancel')}", "owner:home")]),
        parse_mode="HTML"
    )

@R.message(S.premium_add_amount)
async def premium_add_amount(msg: Message, state: FSMContext):
    if not is_main_owner(msg.from_user.id):
        await state.clear()
        return
    try:
        ...
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Please enter a positive integer.", parse_mode="HTML")
        return
    data = await state.get_data()
    uid = data.get("premium_target_uid")
    if not uid:
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} User ID missing. Please restart.", parse_mode="HTML")
        await state.clear()
        return

    # Add premium credit
    await add_premium_credit(uid, amount)

    # Log activity
    d = load()
    log_activity(d, "premium_credit_added", msg.from_user.id, f"User {uid} +{amount}")
    save(d)
    ...
    try:
        await msg.bot.send_message(
            uid,
            f"{em(EMOJI_GIFT, '💎')} <b>Premium Credits Added!</b>\n\n"
            f"You have received <b>{amount}</b> premium credits!\n"
            f"Use them wisely. Contact owner for details.",
            parse_mode="HTML"
        )
    except:
        pass
    ...
    await msg.answer(
        ...
        f"User <code>{uid}</code> got +{amount} premium credits.",
        reply_markup=kb([(f"{sc('back')}", "owner:home")]),
        parse_mode="HTML"
    )

@R.callback_query(F.data == "owner:premium:export")
async def premium_export(cq: CallbackQuery, state: FSMContext):
    if not is_main_owner(cq.from_user.id):
        await cq.answer("🚫 Only Main Owner can use this!", show_alert=True)
        return
    await cq.answer("📤 Exporting premium database...")
    await export_premium_file(cq.bot, cq.from_user.id)

@R.callback_query(F.data == "owner:premium:import")
async def premium_import_start(cq: CallbackQuery, state: FSMContext):
    if not is_main_owner(cq.from_user.id):
        await cq.answer("🚫 Only Main Owner can use this!", show_alert=True)
        return
    await state.set_state(S.premium_import_file)
    await cq.message.edit_text(
        f"{em(EMOJI_GEAR, '📥')} <b>Import Premium Database</b>\n\n"
        f"Please upload a <b>.txt</b> file with the premium credit data.\n"
        f"Each line should be in the format:\n"
        f"<code>USERID=CREDIT</code>\n\n"
        f"<i>This will overwrite the entire current premium database.</i>",
        reply_markup=kb([(f"{sc('cancel')}", "owner:home")]),
        parse_mode="HTML"
    )

@R.message(S.premium_import_file, F.document)
async def premium_import_file(msg: Message, state: FSMContext):
    if not is_main_owner(msg.from_user.id):
        await state.clear()
        return
    doc = msg.document
    if not doc.file_name.endswith('.txt'):
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Please upload a valid .txt file.", parse_mode="HTML")
        return
    ...
    try:
        ...
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Failed to read file: {e}", parse_mode="HTML")
        await state.clear()
        return
    ...
    for line in lines:
        ...
        credit_str = parts[1].strip()
        if not uid_str.isdigit() or not credit_str.lstrip("-").isdigit():
            skipped += 1
            continue
        uid = int(uid_str)
    ...
    # Save the new data (overwrites entire file)
    await save_premium_db(new_data)

    d = load()
    log_activity(d, "premium_import", msg.from_user.id, f"Imported {len(new_data)} users, skipped {skipped} lines")
    save(d)

    await state.clear()
    await msg.answer(
        f"{em(EMOJI_CHECK, '✅')} <b>Premium Database Imported!</b>\n\n"
        f"Imported: <b>{len(new_data)}</b> users\n"
        f"Skipped: <b>{skipped}</b> invalid lines",
        reply_markup=kb([(f"{sc('back')}", "owner:home")]),
        parse_mode="HTML"
    )

@R.message(S.premium_import_file)
async def premium_import_invalid(msg: Message, state: FSMContext):
    await msg.answer(f"{em(EMOJI_CROSS, '❌')} Please upload a valid .txt file (not a message).", parse_mode="HTML")
...
async def main():
    ...
    exporter_task = asyncio.create_task(background_premium_exporter(bot))
    log.info("Background scanner and premium exporter tasks created")
    ...
    try:
        await bot.send_message(
            ...
            f"{em(EMOJI_MONEY, '💎')} <b>Premium Credit System:</b> ACTIVE",
            parse_mode="HTML"
        )
    except Exception as e:
