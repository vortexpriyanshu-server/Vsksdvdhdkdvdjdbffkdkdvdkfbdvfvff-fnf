...
# ========== USER REPLY KEYBOARD ==========
# Real Telegram ReplyKeyboard buttons for NORMAL USERS only.
# Owner/Admin/Reseller panels are intentionally untouched.
USER_REPLY_START = sc("🚀 Start Blast")
USER_REPLY_VIDEOS = sc("📹 Videos")
USER_REPLY_CREDITS = sc("💰 Credits")
USER_REPLY_STOP = sc("🛑 Stop Blast")
...
async def user_reply_keyboard_handler(msg: Message, state: FSMContext):
    uid = msg.from_user.id
    d = load()

    # Do not change Owner/Admin/Reseller behaviour.
    if is_owner(uid, d) or is_admin(uid, d) or is_reseller(uid, d):
        return

    if is_banned(uid, d):
        await msg.answer(
            f"{em(EMOJI_CROSS, '🚫')} <b>Aapko ban kar diya gaya hai.</b>\n"
            f"Admin se contact karein.",
            parse_mode="HTML"
        )
        return
    ...
    if not can_use(uid, d):
        await msg.answer(
            ...
            f"Owner se approval lein.",
            parse_mode="HTML"
        )
        return

    # Reply-keyboard buttons can safely interrupt a user FSM flow.
    await state.clear()

    if msg.text == USER_REPLY_START:
    ...
    if msg.text == USER_REPLY_VIDEOS:
        await send_random_video(
            msg.bot,
            msg.chat.id,
            caption=f"{em(EMOJI_VIDEO, '📹')} <b>Random Video</b>"
        )
        await msg.answer(
    ...
    if msg.text == USER_REPLY_CREDITS:
        credits = get_user_credits(uid, d)
        await msg.answer(
            f"{em(EMOJI_MONEY, '💳')} <b>Your Credits</b>\n\n"
            f"{em(EMOJI_MONEY, '💰')} Balance: <b>{credits}</b>\n"
    ...
    if msg.text == USER_REPLY_STOP:
        async with SESSIONS_LOCK:
            session = USER_SESSIONS.get(uid)
            active = bool(session and session.task and not session.task.done())
            if active:
                session.cancelled = True

        if active:
            await msg.answer(
                f"{em(EMOJI_CROSS, '🛑')} <b>Stop signal bhej diya!</b>\n"
                f"Current sending jaldi ruk jayegi.",
                reply_markup=default_reply_keyboard(),
                parse_mode="HTML"
            )
        else:
            await msg.answer(
                f"{em(EMOJI_WARNING, '⚠️')} <b>Koi active blast nahi chal raha.</b>",
                reply_markup=default_reply_keyboard(),
                parse_mode="HTML"
            )
        return
...
async def cmd_start_deep(msg: Message, state: FSMContext):
    ...
    await msg.answer(card(user_home_text(uid, d)), reply_markup=default_reply_keyboard(), parse_mode="HTML")
...
async def fj_check(cq: CallbackQuery, state: FSMContext):
    ...
    if is_owner(uid, d):
        ...
        await cq.message.answer(card(user_home_text(uid, d)), reply_markup=user_kb(), parse_mode="HTML")
...
async def user_home(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.answer(
        f"{em(EMOJI_GEAR, '⌨️')} <b>User keyboard enabled</b>",
        reply_markup=default_reply_keyboard(),
        parse_mode="HTML"
    )
