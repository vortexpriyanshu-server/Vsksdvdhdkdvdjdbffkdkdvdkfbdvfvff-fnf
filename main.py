...
SUPER_ADMIN_NAME = "@Hitek_bomber_support"
SUPER_ADMIN_LINK = "https://t.me/Hitek_bomber_support"
...
PHOTO_URL = "https://t.me/photo_for_bot90/2"
...
def kb(*rows) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=t, callback_data=c, style=ui_style(t, c)) for t, c in row]
        for row in rows
    ])
...
def user_home_text(uid: int, d: dict) -> str:
    ...
    return (
        f"{line}\n"
        f'<tg-emoji emoji-id="6122954593207853337">✨</tg-emoji> <b>ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ᴏᴜʀ ᴄᴏᴜsᴛᴏᴍ sᴍs ʙᴏᴍʙᴇʀ</b>\n'
        f"{line}\n"
        ...
        f"{line}\n"
        f'<tg-emoji emoji-id="6010253654928397236">🔥</tg-emoji> ᴅᴇᴠ » @LUFFY_49 <tg-emoji emoji-id="6242081133274600789">✅</tg-emoji>\n'
        f"{line}"
    )
...
def number_step_text(credits: int) -> str:
    ...
    return (
        ...
        f'<tg-emoji emoji-id="6204263190319081111">💀</tg-emoji> ʙᴏᴛ ᴍᴀᴅᴇ ʙʏ : @LUFFY_49 <tg-emoji emoji-id="6147565374289220368">😎</tg-emoji>'
    )
...
def owner_kb(d: dict, viewer_uid: int | None = None) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=mode_btn[0], callback_data=mode_btn[1], style=ui_style(mode_btn[0], mode_btn[1]))],
        *(
            [
...
async def cmd_start_deep(msg: Message, state: FSMContext):
    ...
    if is_reseller(uid, d):
        await msg.answer(card(reseller_panel_text(uid, d)), reply_markup=reseller_kb(d), parse_mode="HTML")
        return
    if is_banned(uid, d):
    ...
    if not ok:
        await msg.answer(card(user_home_text(uid, d)), reply_markup=user_reply_keyboard(uid, d), parse_mode="HTML")
...
async def fj_check(cq: CallbackQuery, state: FSMContext):
    ...
    if is_owner(uid, d):
        await cq.message.answer(card(owner_panel_text(d)), reply_markup=owner_kb(d, uid), parse_mode="HTML")
    elif is_admin(uid, d):
        await cq.message.answer(card(admin_panel_text(d)), reply_markup=admin_kb(d), parse_mode="HTML")
    elif is_reseller(uid, d):
        await cq.message.answer(card(reseller_panel_text(uid, d)), reply_markup=reseller_kb(d), parse_mode="HTML")
    else:
        await cq.message.answer(card(user_home_text(uid, d)), reply_markup=user_reply_keyboard(None, None), parse_mode="HTML")
...
async def owner_home(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_owner(uid, d):
        ...
        return
    try:
        await cq.message.edit_text(card(owner_panel_text(d)), reply_markup=owner_kb(d, uid), parse_mode="HTML")
    except TelegramBadRequest:
        pass
...
async def admin_home(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_admin(cq.from_user.id, d):
        ...
        return
    try:
        await cq.message.edit_text(card(admin_panel_text(d)), reply_markup=admin_kb(d), parse_mode="HTML")
    except TelegramBadRequest: pass
...
async def reseller_home(cq: CallbackQuery, state: FSMContext):
    ...
    if not is_reseller(uid, d):
        await cq.answer("🚫 Access Denied!", show_alert=True)
        return
    await cq.message.edit_text(card(reseller_panel_text(uid, d)), reply_markup=reseller_kb(d), parse_mode="HTML")
...
async def owner_premium_import_file(msg: Message, state: FSMContext):
    ...
    if not msg.document:
        await msg.answer(
            f"{em(EMOJI_CROSS, '❌')} Please .txt document upload karo.",
            parse_mode="HTML"
        )
        return
    ...
    if filename != "ᴩʀɪʏᴀɴꜱʜᴜ.json":
        await msg.answer(
            f"{em(EMOJI_CROSS, '❌')} Sirf <code>ᴩʀɪʏᴀɴꜱʜᴜ.json</code> file allowed hai.",
            parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        await state.clear()
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Invalid ᴩʀɪʏᴀɴꜱʜᴜ.json format.", parse_mode="HTML")
        return
...
async def rk_balance(msg: Message, state: FSMContext):
    d = load()
    credits = get_user_credits(msg.from_user.id, d)
    await msg.answer(card(f"{em(EMOJI_MONEY, '💰')} Credits: <b>{credits}</b>\nOwner: {SUPER_ADMIN_NAME}"), parse_mode="HTML")
...
async def rk_refer(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        f"{em(EMOJI_STAR, '👥')} <b>Refer & Earn</b>\n\n"
...
async def rk_history(msg: Message, state: FSMContext):
    ...
    history = d.get("sms_history", {}).get(str(uid), [])[-10:]
    if not history:
        await msg.answer(f"{em(EMOJI_GEAR, '📜')} Abhi koi SMS history nahi.", parse_mode="HTML")
        return
...
async def rk_stop(msg: Message, state: FSMContext):
    ...
    async with SESSIONS_LOCK:
        ...
        if sess:
            sess.cancelled = True
            await msg.answer(f"{em(EMOJI_CHECK, '🛑')} Blast stop request sent.", parse_mode="HTML")
        else:
            await msg.answer("Koi active blast nahi hai.")
...
async def rk_admin(msg: Message, state: FSMContext):
    ...
    uid = msg.from_user.id
    if is_owner(uid, d):
        await msg.answer(card(owner_panel_text(d)), reply_markup=owner_kb(d, uid), parse_mode="HTML")
    elif is_admin(uid, d):
        await msg.answer(card(admin_panel_text(d)), reply_markup=admin_kb(d), parse_mode="HTML")
    else:
